...
def main():
    ...
    # ── [8.5] ⭐⭐ `getOD.ashx` 到底吃不吃日期參數（K線分析線 1712 §1-2 指名要我方測）──
    #   ⚠ 對方明講「本線沒有測」。⛔ 而這一格有時效：官方只留 51 週滾動窗口，
    #     每晚一週，最舊那一週就**永久**掉出去。
    #
    #   ⛔⛔ 判準**只有一條**，就是 CLAUDE.md 第二點①那條：
    #     **回應要自己講出它是哪一週。**
    #   ⚠ 這一族最常見的靜默失敗就是「參數格式錯 ⇒ 靜靜回最新一期」，
    #     而它 200、有列、`資料日期` 欄也有值——⛔ 只是值是**今天那一週**。
    #
    #   ⭐ 而錨點要取**會讓兩種假說分岔**的那一格（三點5）：
    #     求「最新那一週」⇒ 參數真假**都會**回 20260911 ⇒ 那跟沒測一樣。
    #     ⇒ 一律拿**最舊**那一週（20250919）去問。
    say("\n[8.5] ⭐ `getOD.ashx?id=1-5` 吃不吃日期參數（K線 1712 §1-2 指名）")
    say("      判準：**回應自己宣告的 `資料日期`** ＝ 我請求的那一週才算吃")
    say("      ⛔ 不是「有沒有回列」「bytes 夠不夠大」——靜默失敗長得一模一樣")
    _base_dates = []
    _r0, _e0 = B.get(URL, retries=1, timeout=90)
    if not _e0:
        _rows0, _ = parse(_r0)
        _base_dates = sorted({pick(x, DATE_KEYS) for x in _rows0} - {""})
        say(f"      對照組（不帶任何參數）：相異資料日期 {_base_dates}")
    else:
        say(f"      ⛔ 對照組就抓不到（{str(_e0)[:60]}）⇒ 下面的結果不可採信")
    _want = "20250919"          # 下拉選單裡**最舊**那一週
    for _pname in ("date", "dateVal", "firDate", "scaDate"):
        _u = f"{URL}&{_pname}={_want}"
        _r, _e = B.get(_u, retries=1, timeout=90)
        if _e:
            say(f"      {_pname:<8} ✗ {str(_e)[:70]}")
            continue
        _rows, _note = parse(_r)
        _ds = sorted({pick(x, DATE_KEYS) for x in _rows} - {""})
        if _want in _ds and len(_ds) == 1:
            _v = "⭐⭐ **吃**（回應宣告的就是我要的那一週）"
        elif _ds == _base_dates:
            _v = "⛔ **假參數**（回應跟不帶參數時逐字相同 ⇒ 靜靜回最新一期）"
        elif not _ds:
            _v = "⚠ 回應裡抽不到資料日期 ⇒ **判不了**（⛔ 不是「不吃」）"
        else:
            _v = f"⚠ 回了別的日期 {_ds[:3]} ⇒ 要人看"
        say(f"      {_pname:<8} {len(_r):,} bytes｜{_note}｜日期 {_ds[:2]}　{_v}")
    ...
    def _chain(cands):
        r"""找最長的「接得起來」的鏈：下一段的下界 ＝ 上一段的上界 ＋ 1。
