...
WANTED = [
    ...
    # ⭐⭐ 2026-09-15 新增，清單 D2（財報**實際公告日**）。
    #   ⛔ 又是一組**沒有人問過**：這份目錄從頭到尾沒有一列在問重大訊息／申報時程。
    #   ⚠ 而市場情報分析線 1846 自承「上櫃那一半我完全沒碰」，
    #     ⭐ 而我方 `_tpex_probe.txt` 第 67 行早就寫著 `mopsfin_t187ap04_O`
    #     「上櫃公司每日重大訊息」⇒ 我也沒去讀自己家（三點 3.5 ④）。
    #   ⚠ 「每日」這個名字**不是證據** ⇒ 真正要問的仍然是期別欄的相異值分佈
    #     （`mops_probe.openapi_case` 那一份）。
    ("⭐ 重大訊息／財報公告日（申報時程）",
     ("重大訊息", "公告日", "申報", "發言", "說明記者會", "資訊揭露", "公開資訊")),
