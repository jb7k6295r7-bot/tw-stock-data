...
⛔⛔ **而那句話 2026-09-15 被推翻了**（probe 117 實測）：
```
...
⇒ totalCount: **14**            ⭐ 跟本檔說的「上櫃那 14 筆」逐位吻合
⇒ 欄位：恢復買賣日期／證券代號／證券名稱／**最後交易日之收盤價格**／
        **恢復買賣開始參考價**／漲停價格／跌停價格／開始交易基準價／詳細資料
```
⚠ 為什麼漏掉這麼久：那句否定句的**掃描範圍只有 swagger**，
⛔ 而這個端點住在**公告區**（`/www/zh-tw/bulletin/…`）——跟我方天天在用的
`exDailyQ`（上櫃除權息）、`revivt`（上櫃減資）**同一族、同一個形狀**。
⇒ CLAUDE.md 三點①【掃描範圍】：一句否定，成立範圍是什麼。

⇒ ⭐ **這一支仍然有用**：股數倍率是**精確值**（14/14 是分母 ≤4 的整數比），
而官方那份給的是**價格**（最後收盤 vs 恢復買賣參考價）⇒ 兩條路互為外部錨點。
