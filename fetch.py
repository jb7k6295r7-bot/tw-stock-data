...
# 2026-09-03 移出（不要再加回來，除非使用者說要）：
#   8996 高力  三大法人連續 10 個交易日淨賣超，命中 watchlist「連續 5 日法人賣超 → 移出」
#              移出理由是籌碼面不是技術面；它是上市股，日K 與法人歷史在資料庫裡查得到
...
def _kind(code, name=None):
    ...
    ## ⚠⚠ 已知後果（2026-09-16 量到）：**TDR 被代號長度切成兩半**

    「只用代號規則」是刻意的（⛔ 猜名稱更糟），⚠ 而它有一個**看得見的後果**，
    在這裡寫下來是因為下游已經踩到了：

    ```
    （2026-09-16 當時）data/meta/stocks.csv 裡名稱含 `-DR` 的共 10 檔（全部 twse）
      4 碼   9103 9105 9110 9136          ⇒ 落在 **kind="stock"**
      6 碼   910322 910861 911608 911622 911868 912000  ⇒ 落在 kind="other"
    ```

    ⚠⚠ **上面那個 10 是 2026-09-16 的母體，⛔ 不是現況**（⭐ 這一行 09-24 補的：
    一個沒有標日期的數字，讀的人會當成現在的）。2026-09-24 逐檔實測：

    ```
    名稱含 `-DR` 共 **21 檔**（全部 twse）
      4 碼  7 檔（9103 9105 9106 9110 9136 9157 9188）⇒ 全部 kind="dr" ✅
      6 碼 14 檔 ⇒ kind="dr" 6 檔、kind="other" **8 檔**
        那 8 檔（910482 910708 910801 911611 911616 911619 911626 912398）
        共同點：六碼、都已停止交易（last_seen 2016-05 ~ 2021-12）、
        ⛔ 而且【不在】`industry.csv` 也不在 `industry_esb` ⇒ 拿不到 91 ⇒ 掉進下面的⑤
    ⇒ ⭐ 根因是 `kind` 由一份【當前值】清單推出來 ——
      在那份清單拍下來之前就退場的代號沒有列。
    ⇒ ✅ 而母體不受影響：六碼永遠進不了 `kind=="stock"`
      （2026-09-24 實測 `kind=="stock" ∧ market∈{twse,tpex}` ＝ 2,126 檔，其中 DR **0 檔**）
    ⇒ ⚠ 但「要 DR 全集」的地方⛔ 不可以用 `kind=="dr"`（只找到 13／21）
      ⇒ 用名稱含 `-DR`。
    ```

    ⇒ ⛔ 它們**十檔都是台灣存託憑證**，而這支分類器把它們分進兩類。
    ⚠ 後果：`backtest/data.load_universe()`（`kind=="stock"` ＋ twse/tpex ＝ 2,130 檔）
    的「普通股」母體裡**有 4 檔 TDR**。
