...
def main():
    ...
    # ⚠ 已知會紅的一種形狀：daily.yml 一天跑兩班（台北 19:0x 與 **00:1x**），
    #   而 00:1x 那一班常常撞到人事行政總處的頁面**還沒換日** ⇒ pd ＝ 昨天。
    #   ⭐ 規模（CLAUDE.md 三點③，量到 2026-09-16）：
    #     19:0x 那一班 **0／6** 紅｜00:1x 那一班 **2／6** 紅（09-10、09-16）
    #   ⇒ ⛔ 它**不是**「一直都紅」（四點五那條），也**不必**放寬：
    #     判定已經寫成 unknown（⛔ 沒有被當成照常開盤），而同一天 19:0x 那一班會補上。
    #   ⇒ ⭐ 這裡只把**班次**講出來，讓讀到 ✗ 的人不必去翻 main 的歷史版本才知道是哪一種。
    #   ⚠ 而 `holiday_status.csv` 回答不了這個問題：它一天一列、同一天重跑會覆蓋
    #     ⇒ 00:1x 那一列到了傍晚就被當天 19:0x 那一班蓋掉（⭐ 對日曆而言那是對的語意）。
    _shift = "00:1x（⚠ 頁面常常還沒換日）" if now[11:13] in ("00", "01") else now[11:16]
    rl.check("頁面日期就是今天", pd == today,
             f"頁面 {pd}｜今天 {today}｜本趟班次 {_shift}｜⇒ 判定寫成 unknown，"
             "⛔ 不可以當成照常開盤"
             "　⇒ ⭐ 同一天 19:0x 那一班會再問一次（實測 19:0x 0／6 紅）")
    return rl.finish()
...
def _schedule(rl, today):
    ...
    rl.info("⭐ 行事曆涵蓋的年份",
            ...
            + (f"｜⛔ **缺 {len(miss)} 年**：{','.join(miss)}"
               "　⇒ ⚠ 那幾年沒有**開休市公告**這條前瞻來源"
               "（⛔ 這條的價值是**事前**知道哪天不開；⚠ 而它只回得到近幾年）"
               "　⭐ 而那幾年的**交易日曆本身有外部判準在核**："
               "`calendar_twse.csv`（來源 TWSE FMTQIK，逐月一發，"
               "跟建日檔的個股端點完全不同條路）2015~2020 各 242~247 天都在，"
               "而它與 `data/universe/daily/` 的檔名集合**雙向逐日相同**（2,850 天）"
               if miss else "｜✅ 沒有缺口"))
