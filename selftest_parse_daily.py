...
def main():
    ...
    # ══════════════════════════════════════════════════════════
    # ⑩ ⛔⛔ 「回應要自己講出它是哪一天」——每日那一支 2026-09-10 才補上。
    #   ⚠ `today` 是**猜**的（只跳過週六日，不知道國定假日與颱風停市），
    #     而候選清單最後一條 `STOCK_DAY_ALL` **沒有 date 參數**、永遠回最近一個交易日。
    #   ⇒ 兩件事湊在一起 ＝ 把上一個交易日的行情寫進「今天」的日檔，
    #     ⛔ HTTP 200、stat=OK、每個數字都是真的，只是屬於別的一天。
    # ══════════════════════════════════════════════════════════
    ck("⑩ 標題說的就是那一天 ⇒ 採用",
       _F._same_day({"title": "115年09月09日 每日收盤行情"},
                    "2026-09-09") == (True, "title=115年09月09日 每日收盤行情"))
    ok_, said_ = _F._same_day({"date": "20260907", "title": "115年09月07日"},
                              "2026-09-09")
    ck("⑩ ⭐ 回的是 09-07 而我要 09-09 ⇒ **不採用**，而且講得出它說了什麼",
       ok_ is False and "20260907" in said_, f"{ok_}｜{said_}")
    ck("⑩ ⚠ 回應完全沒自述日期 ⇒ 放行（⛔ 不可以因為沒講就整批拒收）",
       _F._same_day({}, "2026-09-09")[0] is True)
    ck("⑩ ⛔ 參數回音打不穿：`date` 抄回我的、但 `strDate` 說了別天 ⇒ 不採用",
       _F._same_day({"date": "20260909", "strDate": "20260907"},
                    "2026-09-09")[0] is False)
    # ⛔⛔ 這一條原本只斷言 `[0] is True`——⚠ 那在「認出來了」與「**抽不出來所以
    #   靜靜放行**」兩種情況下都會過 ⇒ **它自己就是那種不會擋的守門**。
    #   ⭐ 突變測試抓到的：把 `\d{7}` 那條規則整個拿掉，這條斷言**照樣綠**。
    #   ⇒ 判準要同時看 `said`（有沒有真的認出來），跟下面那一段一樣寫法。
    _ok7, _said7 = _F._same_day({"date": "1150909"}, "2026-09-09")
    ck("⑩ 民國 7 碼的自述**要真的認得出來**（⛔ 不是抽不出來所以放行）",
       _ok7 is True and _said7 != "", f"{_ok7}｜{_said7!r}")
    ck("⑩ ⭐ 而且認出來之後真的會擋：`1150907` vs 要 09-09",
       _F._same_day({"date": "1150907"}, "2026-09-09")[0] is False)
    ck("⑩ ⚠ 兩支用的是**同一支** `_same_day`", B._same_day is _F._same_day)
    # ⛔⛔ 守門自己的洞：斜線與連字號的日期原本**抽不出來** ⇒ `said` 留空
    #   ⇒ 這道防線**靜靜地放行**。⚠ 一道「看起來有、實際上沒在擋」的防線，
    #     比沒有防線更糟——它讓人以為這件事已經被守住了。
    for v, want, lbl in (("115/09/09", "2026-09-09", "民國斜線"),
                         ("2026/09/09", "2026-09-09", "西元斜線"),
                         ("115-09-09", "2026-09-09", "民國連字號"),
                         ("20260909", "2026-09-09", "8 碼")):
        ok2, said2 = _F._same_day({"date": v}, want)
        ck(f"⑩ ⭐ `{v}` 要**認得出來**（不是靜靜放行）｜{lbl}",
           ok2 is True and said2 != "", f"{ok2}｜{said2!r}")
    ck("⑩ ⭐ 而且認出來之後真的會擋：`115/09/07` vs 要 09-09",
       _F._same_day({"date": "115/09/07"}, "2026-09-09")[0] is False)
    ck("⑩ ⚠ 區間標題「104年07月16日 至 104年07月16日」不可以被串成垃圾",
       _F._same_day({"title": "104年07月16日 至 104年07月16日"},
                    "2015-07-16")[0] is True)

    # ══════════════════════════════════════════════════════════
    # ⑩之二 ⛔⛔ **西元年月日被當成民國** ⇒ 它算出一個**看起來合理的錯日期**
    #
    #   `feeds:tib` 整段回補失敗，訊息是
    #   `日期不符(title=2021年07月02日每日創新板股票成交量值)`。
    #   ⚠ 表面像「解不出來」，實際上是**解出來了，解成 1932-07-02**：
    #     舊規則是 `(\d{2,3})\s*年`（只認民國），`re.search` 往後挪一格，
    #     從 `2021` 裡抓走 `021` ⇒ 民國 21 年 ⇒ 1932。
    #   ⛔ 而 1932-07-02 是一個**完全合理的日期** ⇒ 沒有任何東西會覺得不對。
    #   ⭐ 代價：這道守門擋掉了 **1,050 天**的創新板回補。
    #
    #   ⚠ 這一族今天出現三種面貌，底層是同一件事：
    #     早上「斜線抽不出來 ⇒ 靜靜放行」（該擋沒擋）
    #     現在「西元被當民國 ⇒ 不該擋卻擋了」
    #     ⛔ 真正的病：**它自己編了一個日期出來，然後拿它去比。**
    # ══════════════════════════════════════════════════════════
    TITLES = (
        ("2021年07月02日每日創新板股票成交量值", "2021-07-02",
         "⭐ 西元年月日**後面直接接文字、沒有空白**（STOCK_TIB 就是這樣）"),
        ("115年09月09日 信用額度總量管制餘額表", "2026-09-09", "民國年月日＋空白"),
        ("2026年09月09日", "2026-09-09", "西元年月日、乾淨字串"),
    )
    for t_, w_, lbl in TITLES:
        ok3, said3 = _F._same_day({"title": t_}, w_)
        ck(f"⑩之二 ⭐ 認得出來而且**不擋**｜{lbl}",
           ok3 is True and said3 != "", f"{ok3}｜{said3!r}")
    # ⛔ 反向：同一批 title 對到**別天**時一定要擋
    for t_, w_, lbl in TITLES:
        bad_ = "2026-01-02" if not w_.startswith("2026") else "2026-01-02"
        ck(f"⑩之二 ⛔ 對到別天要擋得住｜{lbl}",
           _F._same_day({"title": t_}, bad_)[0] is False)
    # ⭐⭐ 這一條是本節的本體：**舊寫法會算出 1932-07-02**
    import re as _re
    _old = _re.search(r"(\d{2,3})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日",
                      "2021年07月02日每日創新板股票成交量值")
    ck("⑩之二 ⭐⭐ 舊規則從 `2021` 裡抓走 `021` ⇒ 民國 21 年 ⇒ **1932**"
       "（⇒ 這一節不是憑空擔心）",
       _old is not None and _old.group(1) == "021"
       and int(_old.group(1)) + 1911 == 1932,
       str(_old.groups() if _old else None))
    ck("⑩之二 ⛔ 而現在的實作算出的是 2021-07-02，不是 1932-07-02",
       _F._same_day({"title": "2021年07月02日每日創新板股票成交量失"},
                    "1932-07-02")[0] is False)
    # ⭐ `(?<!\d)` 那個前置否定在做什麼：**數字串長過 4 碼就整段不認**，
    #   ⛔ 不從中間挑一段出來湊。⚠ 猜一個日期比認不出更糟（第七點）。
    ck("⑩之二 ⭐ `1152021年07月02日`（前面黏著別的數字）⇒ **不認**，"
       "⛔ 不可以從中間挑 `2021` 出來",
       _F._same_day({"title": "1152021年07月02日"}, "2021-07-02")[1] == "",
       repr(_F._same_day({"title": "1152021年07月02日"}, "2021-07-02")))
    ck("⑩之二 ⚠ 而認不出來時是**放行**（無從判斷），⛔ 不是判成不符",
       _F._same_day({"title": "1152021年07月02日"}, "2021-07-02")[0] is True)
