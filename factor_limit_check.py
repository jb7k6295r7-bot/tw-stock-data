...
## ⭐⭐ 退路（`price_limit` 公式）的錯誤率——拿官方 11,106 筆逐位量出來的

```
漲停逐位相同 10,594 / 11,106 = 95.39%   ⛔ 不同 512
    其中 差 > 1%  351   ← ⭐ 現增那一族（基準是「減除股利參考價」）
         差 ≤ 1%  161   ← 捨去／進位方向與官方相反的零星案例
跌停逐位相同 11,099 / 11,106 = 99.94%   ⛔ 不同 7（差 > 1% 只有 2）
```

⇒ ⭐ **跌停側的公式幾乎是對的；⛔ 漲停側對現增案結構上就對不到。**

### ⛔⛔ 而這件事的實際後果全部落在**上櫃**

上市有官方數字 ⇒ 走官方，公式錯不錯不影響判定。
⚠ 而 TWT49U **只涵蓋上市** ⇒ 所有上櫃事件都只有這條退路
⇒ ⛔ **上櫃那 2 筆「超出」，很可能就是同一族的誤報**，
   ⭐ 而我沒有上櫃的官方漲跌停可以驗它 ⇒ 這句話只能寫到「未證實」為止。
"""
import argparse
import csv
import io
