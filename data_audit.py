...
def main():
    ...
    if _cd:
        ...
        # ⛔⛔ 2026-09-10 撤回這一列原本的下半句（「永遠補不回來／沒有辦法」）。
        #   ⚠ 那句是我寫的，而它把「**我查過的那幾條端點**沒有」寫成了「**官方**沒有」。
        #   市場情報分析線照**全站選單**（TPEx 485 條）找，同一天推翻了我兩句。
        #   ⇒ 這一列只留「事實」（那段仍然只有自我一致），
        #     ⛔ 不再替它下「沒有辦法」的結論——那個結論會變成別人放棄的理由。
        L.append(f"| 上櫃交易日曆（{_cd[0]} 之前） | ⛔ **仍然只有自我一致**"
                 f"，共 {len(_older):,} 天 | ⭐ **有辦法了**，見下面那條撤回 |")
    ...
    # ⛔⛔ 撤回兩句「官方沒有」。⚠ 寫在**產生器**裡而不是輸出檔裡——
    #   輸出檔每趟重生，改輸出等於改了個寂寞（而且看起來成功）。
    L += ["", "### ⛔⛔ 撤回兩句我寫過的「官方沒有」（2026-09-10）", "",
          "兩句都是我寫的，兩句都**錯**，而且錯法一模一樣：",
          "**我把「我查過的那幾條端點沒有」寫成了「官方沒有」。**", "",
          "```",
          "① 「櫃買端點只給最近 7 個交易日 ⇒ 那段**永遠補不回來**」",
          "② 「上櫃除權息**沒有歷史來源**」",
          "```", "",
          "⇒ 兩句都被市場情報分析線推翻，而且是**同一天推翻兩次**。",
          "⭐ 他們跟我不同的只有一件事：**照全站選單找**"
          "（TWSE 195 條／TPEx 485 條），**不是照想得到的路徑猜**。", "",
          "```",
          "② 的答案：bulletin/exDailyQ（上櫃除權息，POST）＋ bulletin/revivt（減資，234 筆）",
          "  ⛔ 但 exDailyQ **不是完整的還原來源**：少 17 筆減資 ＋ 5 筆轉上市後事件",
          "     ⇒ 只用它會**刪掉還原因子**。要跟 TWT49U 聯集（見 docs/READ_CONTRACT.md）。",
          "```", "",
          "⇒ ⭐ 規矩已寫成文件（`docs/NEW_ENDPOINT.md` 第 −1 步）：",
          "**只有查過該站完整清單之後，才可以寫「官方沒有 X」。**",
          "可以寫的是「我在 ◯◯ 與 ◯◯ 這兩張表沒找到」，"
          "或「選單 N 條逐條看過，沒有」（附 N）。", "",
          "⚠ 而這兩句的代價**不對稱**：寫「有」錯了會被下一個查證的人抓到；",
          "**寫「沒有」錯了，沒有人會再去查**——它會直接變成別人放棄的理由。"]
