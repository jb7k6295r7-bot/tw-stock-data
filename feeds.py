...
def parse_exright(d, day, known=None):
    ...
    # ⭐⭐ `減除股利參考價`：**不計現金增資稀釋**的那個價。
    #   ⛔ 2026-09-14 訂正：我原本寫「只扣現金股利、不調整配股」——**錯的**。
    #   官方公式是 `（前收 − 息值）/（1 + **無償**配股率）`
    #   ⇒ 它**有**調整配股，只是**只算無償那一半**，⛔ 不含現增的稀釋。
...
FEEDS = {
    ...
    #
    #   ⚠ 這張表**只涵蓋上市**。
    #   ⛔⛔ **2026-09-16 推翻**：上一版這裡寫著「上櫃 14 筆（6548、5314、5904…）
    #     還沒有來源，TPEx 的對應端點尚未找到」——⭐ **找到了**：
    #
    #       POST https://www.tpex.org.tw/www/zh-tw/bulletin/pvChgRslt
    #            startDate=2015/01/01&endDate=<今天>&response=json   ⇒ **14 列**
    #
    #     ⚠ 而它跟 `revivt`（上櫃減資，我方天天在用）**同一個區段**
    #     ⇒ 那句「尚未找到」的掃描範圍其實只有 swagger（三點①）。
    #     ⛔ 而 GET 會**靜默**回「今天～明天」⇒ 必須 POST；判準是它**回顯**我請求的區間。
    #   ⇒ 落地在 `otc_parvalue_history.py`／`data/meta/otc_parvalue_history.csv`
    #     （feeds run 157 實測：14 列，換股率跟 `otcparvalue` 的股數倍率 **14/14 逐位相同**）。
    #
