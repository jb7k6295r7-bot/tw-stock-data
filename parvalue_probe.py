    ...
    for name, url in (
        ...
        for act in acts:
            ...
            for act in acts:
                ...
                # ⭐⭐ 整份印出來（2026-09-15 第五輪）。⚠ 只有 8 KB／14 列，
                #   ⛔ 而「開頭 160 字」寫不出解析程式——CLAUDE.md 第一點：
                #   **接新端點的第一件事是把回應印出來，再開始比對**。
                #   ⇒ 有了它才寫得出 `otc_parvalue_history`，而且假回應才做得出真形狀。
                # ⚠ 而它同時是**證據**：日後那 14 筆對不上時，原始回應就在 repo 裡。
                if len(txt3) <= 40000:
                    say("           ── ⭐ 原始回應整份（⛔ 不是摘要）──")
                    for i in range(0, len(txt3), 200):
                        say("             " + txt3[i:i + 200])
                else:
                    say(f"           ⚠ 太長（{len(txt3):,} 字）⇒ 只印前 4,000 字")
                    say("             " + txt3[:4000])
