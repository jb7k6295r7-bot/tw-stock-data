...
def implied_cash(pre_close, keep, ref_official):
    ...
    ## ⛔⛔ 2026-09-13 補：官方**自己給了那條式子**，而它比我推的多一項

    `_reduce_ratio_probe.txt` 裡 TWTAUU 的 `formula` 鍵（⚠ `_tables()` 一直丟掉它）：

        退還股款：恢復買賣參考價＝（停止買賣前收盤價 − **息值** − 每股退還股款）
                                  /（減資換股率）

    ⇒ ⭐ 兩件我方沒有的：
      ① **息值**——「除息併案辦理減資」的案子會同時扣現金股利
         （TWTAVU 的 notes 自己舉了 2323 中環：每股配發 0.20 元）
      ② 「每股退還股款」是**官方欄位**（TWTAVU 有），⛔ 不是我推的「面額 10 × 比例」

    ⚠ 所以上面那 16 筆對不上的，**其中一部分可能就是除息併案**——
    ⛔ 而我方連那一項都沒有，當然湊不出來。
    ⇒ 這一欄在官方那兩欄回補下來之前仍然只是線索；⭐ 而回補之後它應該被**取代**，
      ⛔ 不是繼續推。（公式的變動由 `official_formula.py` 盯著。）
    """
