...
def main():
    ...
    try:
        ...
        # ⛔⛔ 2026-09-16 probe run 129 付過代價：那一步 failure，⚠ 而我**事後查不出原因**
        #   ——Actions 的 job log API 只回尾段，而那一步在很前面。
        #   ⇒ ⭐ 判準：失敗訊息裡要有 **git 自己說的那句話**（這裡是 hook 的「拒絕（測試用）」）。
        ck("⭐⭐ 失敗訊息裡要有 **git／遠端自己說的原文**"
           "（⚠ 只說「推不上去」= 下一個人還是查不出為什麼）",
           "拒絕（測試用）" in o3, o3[-500:])
        # ⭐⭐ 而**位置**才是重點：原文要在「推不上去」那一段**之後**再出現一次。
        #   ⚠ 每一輪重試時它也會印，⛔ 而那幾行離最後的 exit 4 很遠
        #   ——run 129 就是這樣：我拿到的只有尾段，而原因在前面。
        _i_fail = o3.find("推不上去")
        ck("⭐⭐ 而原文要**再印在最後那段失敗訊息之後**（⛔ 不是只在重試那幾行）",
           _i_fail >= 0 and "拒絕（測試用）" in o3[_i_fail:],
           f"推不上去在 {_i_fail}｜之後 {o3[_i_fail:][:200]!r}")
    finally:
