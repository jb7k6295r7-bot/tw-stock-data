...
def index_of_row(fields, row):
    ...
    **不可以**拿來補 `market_amount.csv`——實測同一天兩條路差很遠：
    ...
    ## ⚠ 而上面那張表只比了**兩條路**——2026-09-15 補量第三條

    ⛔ 「我方」那一欄取的是 **MI_INDEX 的大盤列**。⭐ 而我方其實還有一條：
    `data/universe/daily/` 的 twse 列**自己加總**（同一天 1,377 列）：
    ...
    ⇒ ⭐⭐ **口徑差不是一個籠統的差**：金額與筆數幾乎對得上，⛔ 而股數差 29%。
    ⚠ 而上面那張表寫的「2.5 倍」是**對 MI_INDEX 大盤列**說的
    ⇒ ⛔ 不要把它讀成「FMTQIK 的每一欄都差 2.5 倍」。

    ⇒ 這一段的結論**不變**（⛔ 仍然不可以拿那三欄補 `market_amount`），
    ⭐ 而「差在哪一欄、差多少」現在量得到了
    ⇒ 四條路擺在同一頁上的那一節在 `keys_probe.f2_kou_jing()`
    （⛔ 數字只留在那裡一份，這裡不重抄——四點五）。
    """
    idx = None
