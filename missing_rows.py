...
def day_regressions(new_byday, old_byday):
    ...
    ## ⛔⛔ 而 2026-09-15 那晚證明上面那條**只修了一半**

    ```
    11:26Z（台北 19:26）  2026-09-15 ⇒ 0 筆   ← 官方融資融券清單還沒發完
    16:24Z（台北 00:24）  2026-09-15 ⇒ 8 筆   ← 同一晚第二趟，清單齊了
    ⇒ ⛔ 判成「1 天變差」，⚠ 而**什麼都沒有變差**——是官方清單變完整了
    ```

    ⭐ 病根：「這一天**第一次出現**」與「這一天的**官方清單第一次完整**」
    是兩件事，⛔ 而上面那條只擋掉了第一種。
    ⚠ daily.yml 一天跑兩班（台北 19:00 與 23:59）⇒ 當天那一天**必定**被量兩次，
    而第一趟時融資融券還沒發完（CLAUDE.md 四點五那條 `feeds:margin` 同一個事實）
    ⇒ ⛔ 這道閘門會在**每個交易日的第二班**紅一次，然後被學會忽略。

    ⇒ ⭐ 判準：**上一份快照裡最新的那一天不可比**（它的官方清單還在沉澱）。
    ⛔ 而這不是「放掉它」——③還有誰在守：**隔天那一趟**兩邊都是沉澱完的清單，
    真的退步照樣會紅，只是晚一趟。
    ⚠ 而它只讓掉**一天**：`old_byday` 的最大日期，⛔ 不是「最近幾天」。
    """
    settling = max(old_byday) if old_byday else None
    out = []
    for d, n in sorted(new_byday.items()):
        if d == settling:
            continue            # ⭐ 上一份快照裡最新的那一天：清單還在沉澱
