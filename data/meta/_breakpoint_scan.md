...
```
附表 holes_scan.csv：缺 ≥ 5 日且無事件的洞 1401 個、171 檔；其中 liq_ok 9 個（＝ 進斷點清單的 gap 規則）
母體 2124 檔，斷點 11 個、10 檔
rule
gap          8
price+gap    3
missing_trading_days 分布： {'min': 6.0, '50%': 61.0, 'max': 218.0}

對帳 par_change.csv：24 筆／22 檔 → 已有因子 24、仍為斷點 0、漏抓 0
規則另外抓到（不在 par_change.csv）：11 個、10 檔；其中 price 規則 3 個
