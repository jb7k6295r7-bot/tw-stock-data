...
class _Tables(HTMLParser):
    """把 HTML 拆成 [[[cell, …], …], …]。**不裝 lxml，環境只有標準庫。**

    只做一件事：忠實地把 table/tr/td 拆出來。
    判斷哪張表是資料表、哪一列是表頭，是**呼叫端**的事——
    解析器裡塞判斷會讓「解析失敗」與「判斷錯」混在一起，出事分不出是哪一種。
    """

    def __init__(self):
        super().__init__(convert_charrefs=True)
        self.tables, self._t, self._r, self._c, self._in = [], None, None, [], 0

    def handle_starttag(self, tag, attrs):
        if tag == "table":
            if self._t is not None:
                self.tables.append(self._t)          # 巢狀表：先收外層已累積的
            self._t = []
        elif tag == "tr" and self._t is not None:
            self._r = []
        elif tag in ("td", "th") and self._r is not None:
            self._c, self._in = [], self._in + 1

    def handle_endtag(self, tag):
        if tag in ("td", "th") and self._in:
            self._in -= 1
            if self._r is not None:
                self._r.append(_clean("".join(self._c)))
            self._c = []
        elif tag == "tr" and self._r is not None:
            if self._t is not None and self._r:
...
def parse_revenue(raw, year, month, market):
    """t21sc03 → (rows, header, note)。

    ★ 這張表是**按產業別分段**的：產業別自己是一列（只有一格），
      後面跟著該產業的公司。產業別欄要從那些段落標題帶下來，
      不能指望它出現在每一列裡。
    """
    txt, enc = _decode(raw)
    p = _Tables()
    p.feed(txt)
    out, header, sector, skipped = [], None, "", 0
    for rows in p.tables:
        ...
        for r in rows[hi + 1:]:
            if len(r) == 1 or (len(set(r)) == 1 and r[0]):
                sector = _clean(r[0])              # 段落標題＝產業別
                continue
...
def parse_fs(raw):
    """t163sb04／sb05 → [(表頭, 列)]。一頁可能有**好幾張業別分表**。
...
def main():
    ...
    if not a.run:
        ...
        print("[hist] 看過欄位對得上，再加 --run。470 發的回補不該建立在猜出來的 parser 上。\n")
        if a.kind in ("revenue", "both"):
            for mkt, _ in MARKETS:
                ...
                if err:
                    ...
                    continue
                rows, header, note = parse_revenue(raw, 104, 7, mkt)
                dump(f"月營收 {mkt} 104/7", header, rows, note)
                time.sleep(a.sleep)
        if a.kind in ("fs", "both"):
            for form, _ in FS_FORMS:
                ...
                print(f"   編碼 {enc}｜認出 {len(got)} 張業別分表")
                for hdr, body in got[:4]:
                    print(f"   表：{len(hdr)} 欄 / {len(body)} 列｜"
                          f"前 8 欄={hdr[:8]}")
                    if body:
                        print(f"       首列={body[0][:8]}")
                print()
        print("[hist] 對照重點：")
        print("  ① 月營收的欄位要對得上 data/mops/revenue/2026-07.csv 的後半段")
        print("  ② 財報的業別分表數量與欄位，要與 fs/2026Q2_{ci,basi,bd,ins,fh}.csv 對得上")
        print("  ③ **產業別欄有沒有帶到**（月營收是按產業別分段的，"
              "帶不到的話那一欄會整片空白，而且不會報錯）")
        return 0
