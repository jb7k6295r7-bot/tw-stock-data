...
def main():
    ...
    # ⭐ 而這兩個數字**都**是 TradingView 2026-09-13 實測回的值，差別在那顆 ADJ 按鈕：
    #   ADJ 關 87.99978（＝彌補虧損式，現金沒扣）｜ADJ 開 84.66645481（＝現金型，跟我方同量）
    # ⛔ 我一度把前者當成「TV 的性質」——那是錯的，⚠ 它只是那顆按鈕的一個狀態。
    ck("⭐ 誤套那一式算出的 88.00 ＝ TV【ADJ 關】量到的 87.99978",
       abs(wrong - 87.99978) < 0.01, f"算出 {wrong:.4f}")
    right = R.expected_ref(66.00, 0.75, True)
    ck("⭐ 而對的那一式 ＝ TV【ADJ 開】量到的 84.66645481（⇒ 同一個量，兩邊各自算的）",
       abs(right - 84.66645481) < 0.01, f"算出 {right:.4f}")
