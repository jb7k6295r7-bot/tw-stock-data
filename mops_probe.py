...
def ezsearch_case(out):
    ...
    if a is not None and b is not None:
        ...
        out.append("         ⚠⚠ **這一格沒量到**（至少一發沒回來或不是 JSON）"
                   "　⇒ ⛔ 不算失敗，⛔ **也不算驗過**")
    # ══════════════════════════════════════════════════════════════
    # ⭐⭐ 市場情報分析線 1508 §二裁的三支探針（⛔ 順序是他們定的）
    #   ① `CO_MARKET` 上櫃打不打得到 ⇒ **阻斷項**
    #   ② 歷史深度（2015-03／2012）⇒ 決定 (甲) 有沒有「回補」這個選項
    ...
    # ── ① 阻斷項：上櫃 ──────────────────────────────────────────
    #   ⛔ 他們的理由不是工作量：只有上市的話，兩個市場的「資訊可用日」
    #     會變成兩種口徑，⚠ 而跨市場比較時那個偏差**看不出來**（兩邊都是合法日期）
    #     ⇒ ⭐⭐ 那比「兩邊都用法定期限」更糟 ⇒ **上櫃打不到就整條不接**。
    #   ⚠ 而這支回應**沒有 `params`** ⇒ ⛔ 只能兩市場比內容，不可從參數名推。
    out.append("")
    out.append("      ── ① 【阻斷項】`CO_MARKET`：上櫃打不打得到")
    out.append("         ⛔ 判準是**兩個市場的回應不一樣**（⚠ 沒有 `params` 可以問）")
    _mk = {}
    for _tag, _cm, _cid in (("上市 sii", "sii", "2330"), ("上櫃 otc", "otc", "6488")):
        _mk[_tag] = _shot(f"CO_MARKET={_cm!r}｜CO_ID={_cid}｜step=00",
                          dict(base, step="00", CO_MARKET=_cm, CO_ID=_cid,
                               SDATE="20260301", EDATE="20260331"))
    _a, _b = _mk.get("上市 sii"), _mk.get("上櫃 otc")
    if _a is not None and _b is not None:
        out.append(f"         ⇒ ⭐ 兩市場**{'逐列相同' if _a == _b else '不同'}**"
                   + ("　⛔ **⇒ `CO_MARKET` 是假參數 ⇒ 阻斷項不過，整條不接**"
                      if _a == _b else
                      "　⇒ ⭐ 它有生效；⚠ 而上櫃那一發有沒有列要看上面的筆數"))
    else:
        out.append("         ⚠⚠ **這一格沒量到**（至少一發沒回來）"
                   "　⇒ ⛔ 不算失敗，⛔ **也不算驗過**")

    # ── ② 歷史深度 ───────────────────────────────────────────────
    #   ⚠ 我方只測到 2024。⇒ 往回打 2015-03 與 2012-03。
    #   ⛔ 判準是「那一批自己講得出它是哪一期」：看回來的 `CDATE` 落在哪裡，
    #     ⛔ 不是「有沒有回列」（靜靜回今天是第二點①）。
    out.append("")
    out.append("      ── ② 歷史深度：往回打 2015-03 與 2012-03")
    out.append("         ⛔ 判準是回來的列**自己講出它是哪一期**（看 `CDATE`），"
               "不是「有沒有回列」")
    for _y in ("2015", "2012"):
        _r = _shot(f"step=00｜SDATE={_y}0301 EDATE={_y}0331",
                   dict(base, step="00", SDATE=f"{_y}0301", EDATE=f"{_y}0331"))
        if _r:
            _cd = sorted({str(x.get("CDATE", "")) for x in _r if isinstance(x, dict)})
            out.append(f"         ⇒ {_y}-03：{len(_r)} 列｜`CDATE` 相異值 "
                       f"{_cd[:4]}{'…' if len(_cd) > 4 else ''}")
        elif _r is not None:
            out.append(f"         ⇒ {_y}-03：**0 列**"
                       "　⇒ ⚠ 而「0 列」講不出是「那時沒有」還是「查不到那麼早」")

    # ── ③ 1,000 列上限 ──────────────────────────────────────────
    #   ⛔⛔ 他們要的**不是「窗開多寬」**，是**落地判準怎麼寫**：
    #     ✅ 回傳列數 == 1000 ⇒ 報 ✗ 並自動把窗切半重打
    #     ⛔ 不可以寫成「窗開 N 天」——⚠ 財報季的量是變動的，
    #       一個平常月份夠窄的窗到 3 月就會撞上限，⇒ 而撞上限**不報錯**
    #       ⇒ 那條回補會安靜地少掉一整個財報季的前三週。
    out.append("")
    out.append("      ── ③ 1,000 列上限：⛔ 要的是**判準**，不是窗寬")
    out.append("         ⭐ 判準寫成：**回傳列數 == 1000 ⇒ 報 ✗ 並自動切半重打**")
    out.append("         ⛔ 不可以寫成「窗開 N 天」——財報季的量是變動的，"
               "而撞上限**不報錯**")
    for _tag, _sd, _ed in (("財報季 2026-03 整月", "20260301", "20260331"),
                           ("非財報季 2026-06 整月", "20260601", "20260630"),
                           ("半個月 2026-03 上半", "20260301", "20260315")):
        _r = _shot(f"{_tag}｜step=00｜CO_ID 空（全市場）",
                   dict(base, step="00", CO_ID="", SDATE=_sd, EDATE=_ed))
        if _r is not None:
