...
def main():
    ...
    # ⛔⛔ 這一支**故意只比一個方向**，⚠ 而三點①說「只比一個方向就宣告一致」是事故
    #   ⇒ 所以理由要寫出來，否則下一個人只有兩條路：以為它比過了、或去補一個
    #     **必然天天紅**的反向檢查。
    #
    #   反方向（我方有、官方沒有）在這裡**沒有意義**：官方這一發是**上櫃**當日的
    #   市值排行（888 檔），而我方日檔是**全市場**（含上市、興櫃、ETF…）
    #   ⇒ 我方一定多出一大堆，而那**不是**缺口。
    #   ⭐ 判準：兩邊的**母體定義**一樣時，反方向才是證據；不一樣時它是雜訊。
    #   （對照組：`otc_exright_history` 兩邊都是「上櫃除權息事件」⇒ 反方向要比，
    #     而 2026-09-16 補上之後量到 246 筆、全部解釋得了。）
    rl.info("⛔ 這一支為什麼**不比反方向**",
            "官方這一發是**上櫃**當日市值排行（888 檔），我方日檔是**全市場**"
            "　⇒ 兩邊母體定義不同，「我方有、官方沒有」必然一大堆而且**不是缺口**"
            "　⇒ ⭐ 母體定義相同時反方向才是證據（三點①）")
    return rl.finish()
