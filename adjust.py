...
# ★ 「會停止買賣數個交易日」的事件種類。前收盤交叉核對要用
#   「該檔自己在事件日之前的最後一筆收盤」，不是日曆前一交易日。
...
#                  ⇒ **兩邊都有** ⇒ ⛔ 沒有單邊偏差可修
#
#   ⛔⛔ 2026-09-14 訂正：我原本在這裡寫「**FinMind 的 `value` 是各自捨入的**」
#     ——⚠ 那是**冤枉它**。拿**官方**櫃買公告區的歷史表（13,106 列）量同一件事：
#       ref 較高 **14.15%**｜較低 10.77%｜最大偏差 **±0.0050**（＝半分）
#     ⇒ ⭐ **上櫃官方本身就是四捨五入**（上市是無條件捨去，兩站規則不同）
#     ⇒ FinMind 那 14.05%／11.57% 是**忠實反映官方**，不是它自己在捨入。
#
#   ⇒ 所以「上櫃不套」的理由要改寫成正面的：
#     ⭐ **四捨五入沒有單邊偏差** ⇒ cum_factor 不會累積漂移 ⇒ 沒有東西要修。
#     ⚠ （用 `pre − value` 仍可少一點逐筆雜訊，⛔ 但只有 13.5% 的列有多餘精度，
#        而且 2019 以前**一列都沒有** ⇒ 收益極小、且會讓新舊年份口徑不一致。）
#   ```
...
def last_data_day():
    ...
    ⭐⭐ 這一份是「**判一筆事件算不算未來**」的**唯一**基準（CLAUDE.md 四點五）。

    ⛔ 判準是**我方資料最後一天**，⚠ 不是「今天」——兩者在週末與盤中會差好幾天：

        今天 2026-09-14（週一早上）　我方日檔最後一天 2026-09-11（週五）
        ⇒ 一筆 2026-09-14 的事件，拿「今天」當上界就會被判成**缺口**
        ⇒ ⛔ runlog 寫「那一檔的還原序列是假報酬」，⚠ 而那一天根本還沒收盤

    ⚠ 這個坑已經踩過**兩次**，兩次都是同一句話：
      ① 2026-09-11 `otc_exright_check`：今天才除息的那筆被報成沒落地，
         ⛔ 而我照那句寫信給 K線分析線——**那封信是錯的**。
      ② 2026-09-14 `otc_reduce_history`：6129 普誠 2026-09-14 的減資
         被報成「⛔ 未歸因缺口」，⚠ 而它是官方**預告**、事件當天還沒過完。
      ⇒ ⭐ ①修好的時候②沒有跟上——**那正是「同一件事兩份實作」的形狀**，
        所以現在只留這一份，兩邊都叫它。

    ⚠ 讀不到日檔目錄時退回今天，⛔ **而且要把「退回了」講出來**：
      靜靜退回去 ＝ 判準悄悄變回舊的那一個，而畫面上看不出差別。
    """
    days = trading_days()
    if days:
        return days[-1], False
    from datetime import datetime as _dt, timedelta as _td, timezone as _tz
    return _dt.now(_tz(_td(hours=8))).strftime("%Y-%m-%d"), True
...
def build(code, rows, cal, verify=True):
    ...
    if closes:
        for (d, f, pre, ref, kind, mk, srck, _fo) in rows:
            ...
            #   拿**日曆前一交易日**去比——而它們同樣停止買賣 5~8 個交易日
            #   （實測：面額變更 6~8 天、ETF 分割 5~6 天），
            #   那幾天該檔根本沒有列，比到的不是同一個對象。
            #   ⚠ 這種錯**不會報錯**，只會讓交叉核對變成一片假不符或假跳過。
            #   → 新增一種「停止買賣型」事件時，**要記得加進這個集合**。
            if srck in HALTING_KINDS:
                # ★ 這幾種都會**停止買賣數個交易日**，「停止買賣前收盤價」是停牌前
