...
NO_SOURCE = [
    "⛔ **變更股票面額的還原因子——上櫃那 14 筆**（2026-09-09 收掉上市那一半）。"
    "上市已接上：TWSE `change/TWTB8U`，四項判準全過、參數確定生效，"
    "官方前收盤與 `parvalue_scan.py` 的全庫掃描逐格對上"
    "（8070 長華 190.00→19.00、7780 大研生醫 185.00→18.50，因子都是 0.10）。"
    "⚠⚠ **但全庫 24 筆裡 14 筆是上櫃**（6548 長科 ×10 與 ×2.5、5314 世紀 ×20、"
    "5904 寶雅 ×10…），TPEx 沒有對應端點、還沒找到，"
    "**跨過那 14 筆的長期報酬、均線、扣抵值現在仍然是錯的**。"
    "名單見 `data/meta/_parvalue_scan.md`",
    "（已收掉，留作紀錄）2026-09-08 之前 `data/adj/` 實測"
    ...
    "**實測 24 筆**（14 筆上櫃的股數是乾淨整數倍 ×2/×2.5/×4/×10/×20、"
    "與收盤比互為倒數；另 10 筆上市沒有 shares 欄，靠比值＋名稱＋停牌天數），"
