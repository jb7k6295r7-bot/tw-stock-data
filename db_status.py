...
NOT_YET = [
    ...
    "那 14 筆已於 2026-09-09 還原，但**因子是用股數倍率推導的，不是官方公告**"
    ...
    "它們的 `source` 都是 `universe`／`note=feed-shares`，代表 `tpex-mopsfin-O` 那份快照沒涵蓋到。"
    "下一步（還沒試過的）：查那 9 檔是不是新上櫃／興櫃轉上櫃，"
    "以及 `tpex-mopsfin-O` 是不是有比目前更新的期別",
    # ⚠ 同一輪複查刪掉兩條**已經不存在**的待辦：
    #   ①「487 檔有股本無產業別」→ 實際是 **364 檔而且全是興櫃**，
    #     而 `industry.py` 只涵蓋上市與上櫃 ⇒ 那不是缺口，是**興櫃沒有產業別來源**。
    #   ②「`_hist_status.csv` 6 筆待處理」→ **那個檔根本不存在**（早就沒了）。
    #   ⇒ 兩條都是我自己 todo 上寫死、然後安靜過期的數字。與 breadth 那條同一族。
    "⚠ **興櫃沒有產業別來源**（364 檔）。`industry.py` 的類股名單來自 TWSE `MI_INDEX`，"
    "只涵蓋上市與上櫃 ⇒ 興櫃在 `industry.csv` 裡根本沒有列。"
    "⛔ 這不是「漏填」，是那個來源不涵蓋興櫃。下一步（還沒試過的）："
    "查 MOPS 的興櫃基本資料表有沒有產業別欄",
