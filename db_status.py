...
NO_SOURCE = [
    "⛔ **變更股票面額的還原因子**（2026-09-08 查明缺口）。台股 2014 年起開放"
    "彈性面額，10 元改 5 元會讓股數加倍、股價腰斬，未還原價跨過那天是約 50% "
    "的假跌——與 2026-09-06 之前的減資同一種病。來源 TWSE `change/TWTB8U` "
    "早在 09-06 就被辨識出來，但**只寫成註解、從來沒去抓**；`data/adj/` 實測"
    "只有除權息與減資兩種事件。端點參數與欄位待驗（`parvalue_probe.py`）",
