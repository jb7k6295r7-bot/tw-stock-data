...
NOT_YET = [
    ...
    # ⚠ 「上櫃減資 217 筆沒有官方交叉核對」（2026-09-09 記的缺口）2026-09-19 移出：
    #   下一步早就試過了——`otc_reduce_history.py`（2026-09-10）就是這一掃，
    #   走 `bulletin/revivt`（⛔ 不需要 js），逐筆掃「官方有、我方 data/adj/ 沒有」，
    #   已排進 daily.yml 三段式（先抓→再補→才掃）。⛔ 這裡沒跟著改，是同一種
    #   「答案已經在自己家、db_status 的敘述沒有跟上」——跟 C4 那條是同一天犯的。
    #   現況見 C3（上櫃減資歷史）那一列。
...
LEDGER = [
    ...
    ("C4", "上櫃**面額變更**：自動取得", None, "✅ 完成　" + LEDGER_HAND,
     "⛔⛔ 2026-09-19 訂正：這一格原本寫「真正的解要能執行 js 的環境」，"
     "而那句話在 2026-09-15 就已經被自己家推翻——"
     "`otc_parvalue_history.py` 找到 `bulletin/pvChgRslt` 是一支"
     "**不需要 js** 的 POST／JSON 端點（期間參數會回顯、真的生效），"
     "14/14 對上官方值，⛔ 而這裡沒有跟著改。"
     "⭐ 而「自動偵測新事件、算還原因子」本來就每天在跑"
     "（`otcparvalue.py --run`，股數倍率推導，daily.yml）；"
     "缺的只是**官方交叉核對**那一半沒有排程 ⇒ 2026-09-19 已掛進 "
     "`capital.yml` 月排程（事件很稀疏：11.5 年 14 筆，月頻率夠用）。"),
    ("D1", "月營收／財報三表（`mops/`）", _p_dir("mops/revenue", 1), None,
     "⛔ 端點**只給最新一期**，歷史只能逐期累積。"
     "⛔⛔ 2026-09-15 晚上量到一個**一直在那裡**的缺口（策略線問到才發現）："
     "`data/mops/revenue_hist/`（歷史面板，走靜態頁）裡**一檔 KY 都沒有**"
     "——⭐ 而當期 feed（`data/mops/revenue/`，走 ajax 端點）**有** 122 檔。"
     "⇒ 同一期 2026-07、同樣兩個市場：當期 1,976 檔 vs 歷史 1,851 檔，"
     "差的 **127 檔**裡 **122 檔是 KY**，另 5 檔中 **2 檔是 -DR** "
     "⇒ ⭐ 少的是「**外國發行**」這一整類。"
     "⛔ 而**不是我方濾掉的**：探針解析那張頁 **987 列**、"
     "我方 `2025-01_twse.csv` 也是 **987 列** ⇒ 一列都沒濾。"
     "⇒ 病根在 `mops_history.rev_url()` 寫死 `t21sc03_<年>_<月>_**0**.html`。"
     "⚠ 而「`_1` 是外國企業」是**猜測**（⛔ 第二點⑤：編號不是證據）"
     "⇒ 已派探針打 `sii/otc × _0/_1/_2` 六發，⭐ 結果一落地就接。"
     "⇒ ⛔ 下游影響已回報：策略線的 `rev_hi24` 對那 152 檔全部是補值，"
     "而 K線分析線 2210 §3-3 裁定在這一格有答案之前①型的量級**停住**。"
     "⛔⛔⛔ 而順著它往下挖（策略線 2225 §二點出方向、我方獨立量了一次）"
     "挖到一個**比 KY 大得多**的：**這份歷史面板只收「今天還在交易」的公司**。"
     "⇒ 掃描範圍 `revenue_hist/*.csv` 全 140 期，母體 `stocks.csv` 的 `kind==stock`，"
     "⭐ 而分組用 **`last_seen`**（⛔ 不用 `delisted.csv`——那張表把**市場轉換**"
     "也記成下市：1752 南光「2022 從上櫃下市」而它今天在上市、天天有成交；"
     "6423／5236 同）："
     "**真的停止交易 140 檔 ⇒ 面板出現 0 檔（0.0%）**；"
     "仍在市 2,333 檔 ⇒ 1,844 檔（79.0%）。"
     "⇒ ⭐⭐ 而判準是「它**還在申報**的那些期別裡有沒有它」：**140／140 整檔消失**。"
     "⇒ ⛔⛔ 一家 2025 年下市的公司，連它 2015~2025 **還在市、還在按月申報**"
     "的那些月份也整檔不存在 ⇒ **倖存者偏誤**。"
     "⚠ 而它比 KY 嚴重的地方（策略線 §2-2 的話）：KY 缺的原因是註冊地"
     "——量測日當天就知道；⛔ 這一支缺的原因是**這家公司到 2026 年還在不在**"
     "⇒ **缺值樣式本身編碼了未來**。"
     "⇒ 已加 `mops_probe.survivor_case()`：拿四個**在那一期還在正常交易**的代號"
     "去打那一期的頁，看它在不在 ⇒ ⛔ 那一步才分得開「頁沒有」與「我方沒抓到」"
     "（⚠ 而我方解析不濾已經量過：987 列 ＝ 987 列）。"
     "⭐⭐⭐ **探針回來了（probe run 110），四發全部是「否」** ⇒ 機制**證實**："
     "民國 110 年 6 月那一期的頁（sii 917 列／otc 780 列）裡，"
     "2456 奇力新（成交到 2021-12-28）／1507 永大（到 2022-04-13）／"
     "6251 定穎（到 2022-08-12）／5371 中光電（到 2026-08-21）"
     "**四檔的代號與名稱各出現 0 次**——⚠ 而那一期它們**全都還在正常交易**。"
     "⇒ ⛔ **那張靜態頁本身就是用【產生當下的公司清單】重繪的**，"
     "⛔ 不是我方抓取或解析的問題。"
     "⇒ ⭐⭐ 所以 `revenue_hist` 的倖存者偏誤**在來源就有，補不回來**"
     "——⛔ 它是一個要在登錄裡標著的**母體限制**，不是待辦。"
     "⚠ 而離線還有一條獨立佐證：把 `last_seen` 門檻放寬到含最近幾天"
     "（母體 140 → 160 檔）⇒ 面板出現 0 → **7 檔**，而那 7 檔正是這幾天才停的"
    ...
    ("D2", "財報**實際公告日**", None, "✅ 完成　" + LEDGER_HAND,
     "⛔ 官方 `t187ap06/07` **沒有**逐檔公告日：財報檔裡的 `報表日期`／`出表日期` "
     "相異值只有 **1 種**（＝我方抓取當天 1150914）⇒ 是快照時戳，不是公告日。"
     "⚠ 而這只說得了那兩條端點，⛔ 不等於 MOPS 沒有（三點①：只掃一條不能替別條下結論）。"
     "⭐⭐ 2026-09-15 新線索，⛔ 而它在**我方自己的檔**裡：`_tpex_probe.txt` 第 67 行"
     "早就寫著 `mopsfin_t187ap04_O`｜**上櫃公司每日重大訊息**｜參數 無"
     "——⭐ 而財報公告本身就是一則重大訊息（市場情報分析線 1846 §二）。"
     "⚠ 而那條線自承「上櫃那一半我完全沒碰」，⛔ 而我也沒去讀自己家（三點 3.5 ④）。"
     "⇒ 已把它與推測的上市對應條（`t187ap04_L`，⚠ 標明是**我拼的**）"
     "加進 `mops_probe`。⭐ **兩條都量到了，而且兩條都存在**："
     "上櫃 6 筆、上市 82 筆，欄位有 `發言日期`／`發言時間`／`主旨`／`符合條款`"
     "——⭐ 正是公告日要的形狀。"
     "⛔⛔ 而我第一版據此寫「只給最新一期，沒有歷史」是**量錯了欄**："
     "我看的是 `出表日期`／`Date`，⚠ 那是**我方抓取那一天的快照時戳**，"
     "它**必定**只有 1 種 ⇒ 拿它問「有沒有歷史」，答案永遠是「沒有」，"
     "⛔ 而那不是量出來的。⇒ 真正回答涵蓋期間的是 `發言日期`（**內容日期**）。"
     "⚠ 而這正是清單 F2 那條的同一句：**「這個端點可不可信」問錯了問題，"
     "要問的是「這個【欄】可不可信」**。"
     "⛔⛔ 而我第一版的修法**過頭到另一邊**：同樣是「內容日期」也有**兩種**"
     "——**批次日**（`發言日期`＝這一批是哪一天的，⭐ 只有它回答涵蓋期間）"
     "與**事件日**（`事實發生日`＝這一列在講哪一天）。"
     "⇒ 實測的反證：`t187ap04_L` 一批 82 筆，`發言日期` **只有 1 種**，"
     "而 `事實發生日` 有 11 種、⭐ **最大值 1151103 是未來**"
     "⇒ 涵蓋期間的上界不可能落在未來 ⇒ ⛔ 事件日不是涵蓋期間。"
     "⚠ 而 `發言時間` 是時分秒、根本不是日期（我用子字串比對把它吃進來了）。"
     "⇒ ⭐⭐ **量到的結論**：兩條的 `發言日期` **各只有 1 種**"
     "（上櫃 1150913、上市 1150914）⇒ **都是每日快照，只有一天**"
     "⇒ 跟 E1／G2 同一族：**只能從今天起累積**，"
     "⛔ 歷史那一半仍然沒有免費官方來源——而那正是 FinLab 那筆錢要買的那一半。"
     "（`openapi_case` 現在把三族分開印，六種形狀各有離線斷言；"
     "突變 T1~T3／S1~S3 全中。）"
     "⚠ 而這題要拆成**兩個**問題問（1846 §三）："
     "**前瞻**（每天抓一次、自己累積）與**歷史**（要能用日期參數回溯）"
     "⇒ ⛔ 只回一句「MOPS 有／沒有公告日」會把它們壓成一件事。"
     "⭐ K線分析線 1740 §2-1 指出 FinLab 有兩欄（揭露日／法定期限）"
     "⚠ 而 FinLab 的條款**我方從來沒讀過**——⛔「未裁定」是**沒有人去讀**，"
     "不是「不可以」。⇒ 要動它之前先把條款原文取回來給情報分析線裁"
     "⭐⭐ 2026-09-15 晚上：挖外部 `.js` 那一層挖到 `mop_search.js` 裡有 "
     "`/mops/web/ezsearch_query`（公告快易查）——⛔⛔ **而我一度把它寫成「D2 有進展：查詢端點」，"
     "那是過度解讀**。逐字讀那幾段：① 它的每一處都在**被註解掉的行**裡；"
     "② 而且全部在 `/*跑馬燈*/ getMsg()` 內（首頁那條捲動的最新消息），⛔ 不是查詢表單；"
     "③ 活著的那一行是 `POST /server-java/AjaxCheck` ＋ `pg=ezsearch`，"
     "而 `mops2.js` 裡同一支是 `step=0` ＋ `eval(resp)` ＋ 一小時一次 ⇒ ⭐ 那是跑馬燈／連線檢查。"
     "⇒ ⛔ **「公告快易查」的查詢表單送到哪裡，目前仍然不知道。**"
     "⚠ 而「註解掉的網址」與「現在還活著的網址」是兩件事——⭐ 前者證明它**曾經**存在，"
     "⛔ 不證明它現在答得出來（第二點⑤：名字不是證據，這裡是「舊碼不是證據」）。"
     "⇒ 下一步：把 `around()` 的 cap 放大（3 → 12），讓剩下那幾處也印出來。"
     "⭐⭐ **而放大之後，訂正本身也要訂正**：那幾處裡**有沒被註解的**——"
     "`var url = \"/mops/web/ezsearch_query\";` 出現在 `/* 最新消息 */ proAN()`、"
     "在組長字串那一段、以及 `url_str = …; url = protocol + \"//\" + host + url_str;` "
     "⇒ ⭐ **它是活的查詢端點**。⇒ 而參數名讀得到（⛔ 不是猜的）："
     "`step`／`CO_MARKET`／`CO_ID`／`PRO_ITEM`／`SUBJECT`／**`SDATE`**／**`EDATE`**／"
     "`lang`／`AN`——⚠ `SDATE`／`EDATE` 是**日期區間** ⇒ ⭐ 那正是 D2 缺的「歷史」那一半，"
     "而 `PRO_ITEM` 就是公告項目（C00 財務資料／M00 重大訊息）。"
     "⛔⛔ 而這一格我連錯兩次，兩次的病根**同一個**：只讀了印出來的那幾處，"
     "而輸出自己寫著「…（另 3 處未印）」——⭐ 那行字是我設計來警告我自己的，"
     "⚠ 而我在它還亮著的時候下了兩次結論（三點①：只看一部分就寫成通則）。"
     "⇒ ⛔ 下一步是**真的打一發**並把回應攤開（第一點），⚠ 不是照參數名推它回什麼。"
     "⭐⭐⭐ **打了**（probe 105／106，2026-09-15 20:50 台北）⇒ D2 的「歷史」那一半**有來源了**："
     "`POST https://mopsov.twse.com.tw/mops/web/ezsearch_query`，form-urlencoded，"
     "`step=00`（⛔ 01 是最新消息、02 是項目清單、03 是跑馬燈 JS——四種**回的東西完全不同**）。"
     "⭐ 回 JSON，`data` 十欄：`CDATE`（民國公告日）／`CTIME`（時分秒）／`COMPANY_ID`／"
     "`COMPANY_NAME`／`AN_CODE`＋`AN_NAME`（公告類別，15 種，含**資產負債表**／**綜合損益表**）／"
     "`SUBJECT`（如「114年第2季財務報告書」）／`CODE_NAME`（產業）／`TYPEK`／`HYPERLINK`。"
     "⇒ ⭐ `CDATE`＋`CTIME`＋`SUBJECT` 三欄合起來**就是 D2 要的那個東西**："
     "哪一家、哪一期財報、哪一天幾點幾分公告。"
     "⭐⭐ 而**日期參數有生效**，判準是第一點那條（換一個區間再打一次，比內容）："
     "A `SDATE=20260301 EDATE=20260331` 的 `CDATE` 是 115/03/**27**~31（5 種）；"
     "B `SDATE=20240301 EDATE=20240331` 的 `CDATE` 是 113/03/26~31（6 種）⇒ **兩批不同**"
     "⇒ ⛔ 不是「靜靜回最新一期」（第二點②）⇒ 回得到 **2024 年**。"
     "⛔⛔ **而它有三個量到的限制，⚠ 三個都還沒解**（⛔ 不是「應該沒問題」）："
     "① `data` 在 A 與 B **都剛好 1,000 列** ⇒ ⭐ 那是**上限**，不是「那段期間就這麼多」"
     "（A 的 `CDATE` 只涵蓋 3/27~3/31 **五天**、B 只涵蓋 3/26~3/31 **六天**，"
     "⚠ 而我送的區間是整個三月 ⇒ **前三週被截掉了** ⇒ 回補一定要切成更窄的窗；"
     "⛔ 而截掉的方式是**安靜的**：`stat` 沒有、`total` 沒有，"
     "⚠ 沒有任何一個鍵會講「我截了」——正是第二點①那一族）。"
     "② `TYPEK` 相異值 **1 種：上市×1000** ⇒ ⛔ 這一發**只有上市**。"
     "⚠ 上櫃要走 `CO_MARKET`（參數名在 JS 裡讀得到）⇒ ⛔ **還沒實測**，"
     "⛔ 不可以照參數名推它會生效（第一點：`params` 回顯才算數，⚠ 而這支回應根本沒有 `params`）。"
     "③ `CO_ID=2330` 我送了，⛔ 而回應裡 `COMPANY_ID` 有 **364 種**（A 232 種、B 271 種）"
     "⇒ ⭐ **那個參數被忽略了** ⇒ 逐檔查這條路不通，只能整批抓回來自己篩。"
     "⇒ ⛔⛔ **而「要不要真的接成供料」不是我的決定**（第五點：換供料是換維護者的決定）"
     "⇒ 已把上面這三條連同實測輸出寄給市場情報分析線裁。"
     "⭐⭐ 2026-09-21：市場情報分析線 20260921-1035 §五**改了完成的定義**"
     "（取代 2053 §一③「遞迴切分做出來才算完成」）："
     "**D2 完成 ＝ 對任意指定區間，能回報【這一段有沒有被截斷】，⛔ 不要求取得完整資料**"
     "——理由：撞頂是**可偵測**的（剛好 1,000 列），把它變成顯性旗標比追求完整值錢，"
     "因為「不完整而不自知」才會污染下游，「不完整但標出來了」只是覆蓋率問題。"
     "⇒ `mops_probe.split_window_avoiding_cap()` 回的 `still_capped` 就是那個旗標"
     "（離線單元測試已驗：單日本身爆量會列進 `still_capped` 而不是被吞掉、"
     "`None` 不當作撞頂），⇒ **照新定義，這一格判完成**。"
     "⚠ 而「加第二個維度切（`TYPEK`／代號範圍）」市場情報分析線**不派**"
     "——D2 維持「精度提升、不急」，⛔ 不要看到這裡就自己動手加。"),
    ...
    ("E2", "集保股權分散：**歷史回補**（原本只求 51 週）",
     ...
     "⛔ 而起點其實更早（2017-07-21 起，另有 10 週 `.csv`／`.xlsx`，"
     "與 2019+ 那批**同一個發布者**、不同原始格式）——那 10 週是**離散樣本**"
     "不是連續序列，且**從未被任何外部來源核對過**（市場情報分析線 20260921-0110）"
     "⇒ `tdcc.hist_gate()` 的 runlog 已逐趟標出這一段。"
     ...
     "⭐⭐ 2026-09-15 晚上跟市場情報分析線**對完帳了**（他們 1842 附件的 23 個檢查點）："
     "`總人數`／`總股數`／`股數_L12to15`／`股數_L13to15` **四個整數欄 23 週逐位相符**。"
     "⛔ 而兩個 `比例%` 欄 **0／23**——⚠ 而那**不是錯**，是兩種不同的百分比："
     "他們的是**從股數推導**（24,293,492,882 ÷ 25,930,380,458 × 100 = 93.687375，逐位吻合），"
     "我方的是**官方 `pct` 欄逐級相加**，而官方只給兩位小數 ⇒ 加不出四位小數。"
     "⇒ ⭐ 這是 K線〈七十〉那條換了主詞：**沒寫下定義的數字只能被取代，不能被對帳**"
     "——⚠ 而比例比計數更難看出來：兩邊都叫「比例%」、都在 0~100，"
     "⛔ **差 0.02 讀起來就像四捨五入**。⭐ 把它定住的是「算一次除法」。"
     "⇒ 往後跨線交換這一欄要帶定義（`pct_derived`／`pct_official_sum`）。"
     "⭐⭐ 而 371 vs 370 也解掉了，⛔ **不是計數定義**：差集恰好一個 `2023-10-20`，"
     "我方是**故意不收**的（那份被截斷）⇒ ⚠ 反過來說**他們那份的那一週少了一千多檔**，"
     "⛔ 而 2330 在檔案前段 ⇒ 用 2330 那條線看不出來（已回報）。"
     "⭐ 而 `tdcc_hist/` 原本**沒有任何常駐守門**（`verify_hist` 只在 --import-hist 跑一次）"
     "⇒ 已加 `tdcc.hist_gate()`：週數只准往上 ＋ **缺口棘輪**（7 週，每一筆都寫成因，"
     "⚠ 其中 2022-10-31 仍然是「不明」而且自測釘著那兩個字不准被人補成好聽的理由）。"
     "⛔ 而缺口用的是 **ISO 週連不連續**，不是「相鄰日期差 > N 天」"
     "——後者實測報 **85 個**，⚠ 其中絕大多數是快照日位移（6 天＋8 天＝14，一週都沒少）"
     "⇒ 那會是一道天天紅的閘門。ISO 週法報 7 個，每一個都真的缺。"),
    # ⭐⭐ 2026-09-15：這一格解掉了，⚠ 而**不是因為找到了級距文字**
    #   （來源仍然只給代碼 1~17）——是 370 週落地之後它變成**夾得出來**的。
    ("E3", "集保**級距對照**（第幾級＝幾張）",
     _p_rows("meta/tdcc_levels.csv", 15), None,
     "⭐⭐ **從我方自己的 1,538 萬列夾出來的**，⛔ 不是抄坊間表"
     "（`tdcc_probe` 自己寫著那是間接證據）。推導："
     "平均持股（股數÷人數）必定落在該級區間內 ⇒ "
     "`max(avg_k) ≤ b_k ≤ min(avg_{k+1}) − 1`。"
     "⇒ **7/14 個上界夾成唯一解**，其餘寬度 ≤ 2 股（級 10 例外，89 股），"
     "⭐ 而 14 格**全部包含**坊間表那個數字。"
     "⛔⛔ 而**下界也是區間**（`a_{k+1} = b_k + 1`，而 b_k 可能沒夾死）"
     "⇒ 表上 `exact` 標的是「**兩端都**夾死」，只有 4 級（1／7／8／12）。"
     "⇒ ⭐⭐ 剛好 1,000 張（1,000,000 股）在**第 14 級**、超過 1,000 張是第 14~15 級"
     "（⚠ 差 1 股沒夾死）⇒ ⛔ 「千張大戶 ＝ 1,000 張**以上** ＝ 第 15 級」是**錯**的；"
     "剛好 400 張在第 11 級、超過 400 張是第 12 級（⭐ 這個邊界是唯一解）。"
     "⇒ 問「幾股在第幾級」走 `tdcc.level_of()`，它回**兩個**級別。"
     "⚠ 前提：`人數` 是持有人數、15 級互斥相鄰——⛔ 本庫**不宣稱**這兩條被證明過"),
    ...
    ("F2", "大盤**成交金額／股數／筆數**歷史", None, "⛔ 卡住　" + LEDGER_HAND,
     ...
     "⭐ 而指數欄逐位相同 ⇒ 問題是**欄**不是端點。"
     "⭐⭐ 2026-09-15 晚上（probe run 107）把算術做出來了，⛔ 而結論跟 `notes` 講的相反："
     "2026-09-01 我方 twse 1,377 列 vs FMTQIK —— "
     "金額 **−0.40%**（差 4,672,787,966）、筆數 **−2.6%**（差 138,164）、"
     "⛔ 股數 **−29%**（差 3,765,739,000）⇒ ⭐ **口徑差不是一個籠統的差**："
     "它落在不同欄、幅度差兩個量級。"
     "⭐⭐ 而把**差額自己的隱含均價**算出來，形狀就出來了："
     "4,672,787,966 ÷ 3,765,739,000 = **1.24 元/股**，"
     "而我方本身是 1,182,898,779,151 ÷ 9,235,110,196 = **128.09 元/股** ⇒ 差 **103 倍**；"
     "缺口的每筆均量 27,256 股而每筆均額只有 33,821 元（我方 229,082 元）"
     "⇒ ⛔ **那不是普通股的形狀**。⚠ 領先的候選是**權證**"
     "（我方母體刻意排除、FMTQIK 含），⛔ 而我**沒有直接量到**"
     "（我方庫裡沒有權證的量）⇒ ⚠ 這一句是**未驗**的候選，不是結論。"
     "⛔ 而 FMTQIK 的 `notes` 說「含鉅額交易」——**算術不支持它當主因**："
     "2026-09-09 的鉅額（BFIAUU）股數 55,095,672，"
     "⭐ 只有那個缺口的 **1.5%** ⇒ 解釋不了 29%。"
     "⚠ 這正是第二點④那一句：**`notes` 是說明文字，不是規格**。"
     "⛔⛔ 而這一節自己有一個錨點錯誤（已修）：FMTQIK 一發回的是**整個月**（11 列）"
     "⇒ 原本只印「逐欄合計」＝ 整月合計，⚠ 而並排的①是**單日** ⇒ 兩個數字不是同一個量。"
     "⇒ `keys_probe._row_for_day()` 把測試日**那一列**單獨挑出來（民國與西元都認）。"
     "⚠ 順帶：`MI_INDEX4` 回 **404**（747 bytes HTML）⇒ 那一條沒量到，"
     "⛔ 而它跟「官方沒有這條」在紙上一樣——⭐ 探針印了前 300 字所以分得出來。"
     "⇒ ⛔ 而「要不要拿它回補」仍然是**換供料**（第五點）⇒ 歸市場情報分析線"),
    ("G1", "交易日曆（獨立判準）", _p_rows("meta/calendar_twse.csv", 2800), None,
     "TWSE FMTQIK，與建日檔的個股端點不同條路"),
    # ⭐⭐ 2026-09-15：這一格從「⛔ 卡住」改寫，⚠ 而**不是**因為找到了歷史
    #   ——是因為原本那句「卡住」是在一個**只量過兩個頁**的範圍上下的（三點①），
    #   而我方自己的 `_site_inventory.txt` 早就列著六條。實測結果見下。
    ("G2", "**長期停止買賣**：從今天起累積", _p_rows("meta/longhalt.csv", 1), None,
     "⚠ `suspend.csv` 收的是**暫停交易**（TWTAWU／sprcHis），"
     "10,034 列裡 9,594 列是權證 ⇒ ⛔ 它不是這一格要的東西。"
     "⭐⭐ 2026-09-15 擴充探針（六條全部照抄自 `_site_inventory.txt` 的官方選單）實測："
     "① 上櫃 `tpex_cmode` **20 列**、有 `SuspensionOfTrading` 欄（Ｙ×4）"
     "＋變更交易／分盤／管理股票三個旗標 ⇒ ⭐ 這才是這一格要的類別；"
     "② 上櫃 `mopsfin_t187ap26_O` 回**真的一列**"
     "（4804 大略-KY｜`停止買賣開始日` 1150903）；"
     "③ 上市 `t187ap26_L` 欄名有 `停止買賣開始日`，⛔ 而這一發**每一欄都是空的**"
     "（＝目前沒有案件；⚠ 那跟「端點壞掉」長得一樣 ⇒ ⛔ 未確認它有案件時給什麼）；"
     "④ `suspendListingCsvAndHtml` **265 列、民國 090~115**，命中靶子 1/6（6131 鈞泰）"
     "⇒ ⚠ 那是**終止上市**清單，不是停止買賣。"
     "⛔⛔ 而 ①②③ 的 `Date` 相異值都只有 **1 種**（1150914）"
     "⇒ **它們是快照，沒有歷史**（第二點⑤：陣列型只能靠相異值分佈問這件事）"
     "⇒ ⭐ 跟 E1 同一種：**只能從今天起累積**。"
     "⚠ 而 ②③ 的涵蓋範圍要標清楚：只收「**經營權異動且營業範圍重大變更**」那一類，"
     "⛔ 不是全部的長期停止買賣。"
     "⇒ ✅ **2026-09-17 0020 已裁：接成每日累積**（`longhalt.py`，掛 daily）"
     "——理由是那三條只有當日 ⇒ 每天不抓就是永久損失。"
     "⛔ 而三個限定一條都不可以省：① 逐列帶**抓取日**（first_asof／last_asof／n_obs）；"
     "② ⭐「現行有效案件」**不是事件流** ⇒ 去重鍵分兩種"
     "（`t187ap26` 有開始日 ⇒ 鍵含開始日；`cmode` **沒有任何日期欄** ⇒ 鍵用那一組旗標，"
     "⚠ 旗標變了是新的一段，而「換了一筆案件」我方**分不出來**）；"
     "③ ⛔ 保留期**未驗** ⇒ runlog 每趟重印「觀測 N 天｜單筆最多出現 M 個抓取日 "
     "⇒ **至少**保留 M 天、**上限未知**」——⛔ 不可以宣稱歷史補得回來。"),
