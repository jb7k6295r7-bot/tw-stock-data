...
# ⭐⭐ 白名單每一列要寫**三件**（市場情報分析線 2026-09-11 19:00 指定）：
#   ① 裁定者與日期　② 理由　③ **重審觸發條件**
# ⚠ 原話：「否則三個月後沒人知道這是裁過的還是漏掉的
#   ——**那正是「消失」的另一種形式**。」
GRID_WAIVED = {
    ("emerging", "stock"):
        "✅ **已裁定：不補**（裁定者：市場情報分析線，2026-09-11）。"
        "興櫃 363 檔一律**明示不適用**還原因子，⛔ 不是缺口。"
        "｜理由①（硬）**基準價選不出來**：興櫃只有最高／最低／加權均價／"
        "最後成交價，⛔ 沒有開盤價、沒有真正的收盤價（契約 5.v）。"
        "加權均價不是收盤價（跨日相除得到的不是持有報酬）；"
        "最後成交價在整天只有零星幾筆時**雜訊比訊號大** ⇒ "
        "做出來會是「看起來能用、實際不可信」的數字，"
        "⛔ **而那比沒有更危險——沒有會讓人去問，錯的不會**。"
        "｜理由② 沒有下游會用（興櫃 2026-09-06 起整批排除在推薦母體外）。"
        "｜理由③ 它是**問錯問題**，⛔ 跟上櫃 ETF 缺 18 檔那格不同"
        "（那格基準價明確、有配息就有除息 ⇒ **是真缺口，要補**）。"
        "｜⚠ **重審觸發條件**：興櫃交易制度正在改"
        "（櫃買公告：自民國 115-09-02 起陸續施行，115-12-07 起盤中零股與整股同步開盤）"
        "⇒ ⭐ **`universe/daily` 的 emerging 列一旦出現非空的 `open`，"
        "本裁定立即失效、要重審**（連帶「興櫃不進推薦母體」那條也要重審）。",
}


def emerging_has_open(daily_dir=None, days=None):
    """⭐ 情報分析線指定的**可自動偵測**重審觸發條件。

    → `(有沒有, 說明)`。興櫃列一旦出現非空的 `open`，
    就代表交易制度改到「有開盤價」了 ⇒ ⛔ 上面那條裁定立即失效。

    ⚠ 只看**最近幾天**：⛔ 全庫掃 2,850 個日檔只為了一個布林值太貴，
      而制度改變是**往前發生**的 ⇒ 看最新的那幾天就夠。
    """
    d = daily_dir or DAILY
    if not os.path.isdir(d):
        return False, "⛔ 讀不到日檔目錄 ⇒ **測不出來**（⚠ 不是「沒有」）"
    ds = sorted(x for x in os.listdir(d) if x.endswith(".csv"))[-(days or 5):]
    if not ds:
        return False, "⛔ 日檔目錄是空的 ⇒ **測不出來**"
    seen = 0
    for fn in ds:
        with io.open(os.path.join(d, fn), encoding="utf-8",
            ...
            try:
                mi, oi = hdr.index("market"), hdr.index("open")
            except ValueError:
                continue
            mx = max(mi, oi)
            for ln in f:
                c = ln.rstrip("\n").split(",")
                if len(c) > mx and c[mi] == "emerging":
                    seen += 1
                    if c[oi].strip():
                        return True, (f"⭐⭐ {fn[:-4]} 的興櫃列出現非空 `open`"
                                      f"（{c[oi]}）⇒ ⛔ **裁定失效，要重審**")
    # ⛔ 0 也要講出母體：沒有興櫃列的話這個「沒有」什麼都沒證明（第七點）
    return False, (f"最近 {len(ds)} 天的興櫃列 {seen:,} 列，`open` 全部是空的"
                   "　⇒ 裁定仍然成立"
                   if seen else
                   f"⚠ 最近 {len(ds)} 天**一列興櫃都沒有** ⇒ ⛔ 這個「沒有」"
                   "不能當判準（第七點：0 要附正例數）")
...
def main():
    ...
    # ⭐⭐ 情報分析線 2026-09-11 指定：裁定要帶**可自動偵測的重審觸發條件**
    #   ⛔ 寫在白名單字串裡只是說明——**要有人真的去測它**，
    #   ⚠ 否則「制度改了」這件事只能靠有人記得去看公告。
    _open, _owhy = emerging_has_open()
    rl.check("⭐ 興櫃仍然沒有開盤價（⇒ 「不補還原因子」那條裁定仍然成立）",
             not _open,
             _owhy + ("　⇒ ⛔ **請通知市場情報分析線重審**"
                      "（連帶「興櫃不進推薦母體」那條）" if _open else ""))
