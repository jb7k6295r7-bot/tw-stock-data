...
# ══════════════════════════════════════════════════════════════════
# ⭐⭐⭐ 2026-09-20 回測線 0145 問到：`asof` 是什麼意思？—— 這裡寫死答案。
#
#   `asof` ＝ **抓的那一天**（`fetch_one(sid, today)` 的 `today`，寫入時的
#   台北日期）——⛔ **不是**「官方資料涵蓋到那一天」。
#
#   官方回應反映的是「抓取當天**之前**最後一個交易日」為止的資料 ⇒
#   ⭐ **當年那一列（尚未收官的最新一年）是一張快照**：它不含 `asof` 當天，
#   更不含之後任何一天。⛔ 已完結的年份（`roc_year` < 今年）沒有這個問題
#   ——它們的資料本來就不會再變多。
#
#   ⭐⭐ 回測線 0145 §二實測（`official_yearly_close.csv` 民115 列，
#   `asof=20260916`）：拿我方日檔算到 09-15（asof 前一個交易日）**0 不符**、
#   算到 09-16（asof 當天）767 不符、不切（到 09-18）843 不符
#   ⇒ **官方那一列不含 asof 當天**。
#
#   ⇒ ⭐ **任何拿「當年」那一列去跟我方逐日資料比對的呼叫端**，都要先把
#   我方資料**退到 asof 前一個交易日**，⛔ 不可以直接比到「今天」——
#   否則「當年」那一格會被抓取之後才發生的交易日污染，而且**看起來像
#   我方資料壞了**（回測線 0145：3→846／0→18／0→52 都是這個成因，
#   ⛔ 不是程式改到那裡、也不是資料真的壞了）。
#   ⚠ 這一段目前**沒有共用實作**：回測線在自己的 `backtest/audit_db.py`
#   落地了 `official_asof_cap(official, cal)`（9 條自測、9/9 突變全中），
#   本庫目前沒有會做「當年逐日比對」的呼叫端，⛔ 若之後要加，
#   asof-cap 這一步不可以省。
#
#   ⚠ 而 2026-09-16 建立 `avg_close_expected()` 那條三段進位規則的
#   17,832 個 (檔,年) 母體測量**沒有踩到這個坑**——那次是在 09-16 當天量的，
#   我方日檔那時還沒有超前 asof，⛔ 但**日後重跑同一個測量**若不加這一層退避，
#   會把「當年」那幾格的鑑別力污染掉（民115 那格會多出一批假不符）。
#
