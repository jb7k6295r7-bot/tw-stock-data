    ...
    return [
        ...
        ("⚠ 這個端點答不出來的（⛔ 不是缺口，是涵蓋範圍）",
         ...
         + "　⇒ ⭐ 實測命中率 twse 94%／tpex 0%／emerging 0%"
           ...
           "——那句話講不出它是哪一種）"
         + "　⇒ ⚠ 上櫃的官方年度／月統計這一條**還開著**，"
           "⛔ 而 2026-09-16 起它不再是「沒有線索」："
           "⭐ 我方自己的 `data/meta/_site_inventory.txt` 就列著"
           "**上櫃／交易資訊／盤後資訊／個股月成交資訊**（`stock-month.html`）與"
           "**個股年成交資訊**（`stock-year.html`）"
           "⇒ `tpex_probe.py` [14] 正在量它的資料端點"
           "（⚠ 選單上有**不等於**端點存在，第二點 ⇒ 量到再改這一句）"),
    ]
