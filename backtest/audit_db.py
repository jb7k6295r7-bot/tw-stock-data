...
def _qty_note(j: pd.DataFrame) -> np.ndarray:
    ...
    ⛔⛔ 這只講「官方有而我方沒有」的那一批，⛔ 不講我方的涵蓋：我方上市日檔 volume 93.7% 的列不是 1,000 的倍數（2026-09-16 15:42 本線實測；資料庫線 1600 抽樣 95.7%、1615 全量 94.3%）
    ...
    ⚠ 上櫃日檔 volume 0／505,354 列非千倍數（資料庫線 1615）＝只有一般交易 ⇒ 上櫃的量差是另一件事（可能是零股＋盤後定價＋鉅額，候選），§G 對上櫃只描述比值。"""
...
def official_monthly_tpex_check(res, official: pd.DataFrame) -> pd.DataFrame:
    ...
    volume_kshares（仟股）／amount_kntd（仟元）／transactions ⇒ 我方上櫃日檔只有一般交易（0／505,354 列非千倍數）⇒ 只算比值分佈、⛔ 不判。"""
