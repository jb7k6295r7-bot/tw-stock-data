...
def main():
    ...
    for r in cmp.itertuples():
        L.append(f"| {r.型} | {getattr(r, '本線 kmeans_idx')} | {getattr(r, '本線 n'):,} | {getattr(r, '策略線 n'):,} | "
                 f"{getattr(r, 'n 差'):+d} | {getattr(r, 'p05 差'):+.2f} | {getattr(r, 'p10 差'):+.2f} | "
                 f"{getattr(r, 'p50 差'):+.2f} | {getattr(r, 'p90 差'):+.2f} | {getattr(r, '平均 差'):+.2f} | "
                 f"{getattr(r, '最大分位差'):.2f} | {r.狀態} |")
