# -*- coding: utf-8 -*-
"""PREREG營飆停損 的獨立查核（⛔ 不 import 主程式）：只讀 body_seeds_arms.csv、body_cells.csv 與 resultsN17/regime_t1/seeds.csv，
重算每臂年化中位、回落中位、比值、對 0050 標籤、對營飆 v1 配對計數與判語，逐項比對。輸出同目錄 body_check.json；不符 ⇒ 結束碼 1。"""
import argparse
import json
import os
import sys
...
def main():
    ...
    ap.add_argument("--dir", default=os.path.join(HERE, "resultsYfStop"))
    a = ap.parse_args()
    A = pd.read_csv(os.path.join(a.dir, "body_seeds_arms.csv"), dtype={"eq_sha": str}, float_precision="round_trip")
    TB = pd.read_csv(os.path.join(a.dir, "body_cells.csv"), float_precision="round_trip").set_index("arm")
    ...
    if not out["base＝regime_t1"]:
        out["bad"].append("base")
    json.dump(out, open(os.path.join(a.dir, "body_check.json"), "w", encoding="utf-8"), ensure_ascii=False, indent=1)
