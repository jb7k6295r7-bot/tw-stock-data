...
def main():
    ...
    for arm, g in df.groupby("arm", sort=False):
        ...
        rows.append({"arm": arm, "cagr": md["cagr"], "cagr_p10": g["cagr"].quantile(.1), "cagr_p90": g["cagr"].quantile(.9),
                     ...
                     "slot": md["slot"], "m": md["m"], "maxw": md["maxw"], "maxw_worst": g["maxw"].max(),
    ...
    L = report(T, EV, stat, sig, kinds, MU)
    open(os.path.join(a.out, "P9B_REPORT.md"), "w", encoding="utf-8").write("\n".join(L) + "\n")
    log(f"寫入 {os.path.join(a.out, 'P9B_REPORT.md')}")


def report(T: pd.DataFrame, EV: pd.DataFrame, st: dict, sig: pd.DataFrame, kinds: dict, MU: pd.DataFrame) -> list[str]:
    ...
    L = ["# PREREGP9 2-C ⓑ：市場狀態減碼（回測線落地）", "",
         ...
         f"H120＝持有 {P7.HOLD_BARS_N} 根。種子 `default_rng({SEED0} + r)`、R={len(T) and int(z['m']) and 200}。", "",
         ...
         "| 組 | 年化 中位 | p10～p90 | 最大回落 中位 | p10～p90 | 槽位 | 筆數 | 單一部位最大佔比 | 本線口徑 | 策略線口徑 | 逐種子通過率 |",
         "|---|---:|---|---:|---|---:|---:|---:|:--:|:--:|---:|"]
    for r in (z, b):
    ...
    L += ["", f"⇒ **ⓑ {'✅ 通過' if (b['win_self'] and b['win_sl']) else '⛔ 沒有通過'}**"
          ...
          f"⚠ 槽位使用率 {z['slot']:.2f} → {b['slot']:.2f}：⭐ 掉的那一塊就是【資金閒置】"
          f"⇒ 與 PREREGP3 交叉引用，⛔ 年化的下降不可全部歸因於「減碼本身」。",
    ...
    L += ["", "⛔ 依〈九十八〉：有效樣本數 ＝ 事件數 ＝ %d ⇒ 結論一律寫「在這 %d 次事件上」，⛔ 不寫「測得出／測不出」。" % (len(EV), len(EV)), "",
          ...
          "⭐ 與策略線 §7-2（30 顆種子、133 個事件：混合型 127／單日暴跌型 6／延續下跌型 0）同一個形狀 ⇒ "
          "⛔ 【兩型各報一次】在本庫**退化成幾乎只有混合型**。⚠ 這個退化本身要寫出來，"
          "⛔ 否則下一個人會以為兩型各自有證據。", "",
          "## 五、§2-E⑤ 代理的重疊率（⛔ 兩個方向）", "",
