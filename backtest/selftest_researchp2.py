...
def t_p16():
    ...
    # ⭐⭐ 第三種情形【已裁】（K線 1703 §二／策略線 解讀 seq=2 §3-3）⇒ 斷言改成【定版措辭逐字在不在】
    #   ⛔ 這一條的重點是「回測線有沒有一字不差地照抄」，⛔ 不是「有沒有寫得像」
    for w in ["比登錄設想更強的形式", "E1 顯著劣於 R1", "本身額外造成了損害",
              "不可沿用『改善來自持有時間變短本身』", "本身額外有害，而非中性"]:
        check(o4[0] is False and w in o4[1], f"⭐ 第三種情形定版措辭逐字：「{w}」")
    for w in ["為正 60 顆", "CI[−3.16, −1.63]", "755 筆", "不可據它宣告反向做法", "不可出現「改善」二字"]:
        check(w in o4[1], f"⭐ 策略線四條披露逐字：「{w}」")
    check("〈一百二十五〉" in o4[1] and "〈七十八〉" in o4[1],
          "⭐ 兩條限制一併附上：(i) 的獨立性沒退化（〈一百二十五〉）／共用選股種子（〈七十八〉）")
