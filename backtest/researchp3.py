"""PREREGP3：閒置資金放 0050 vs 空手（策略線 09-15 13:35 丙節；落地版定義見 backtest/PREREGP3.md 追加）。
    ...
    python3 -m backtest.researchp3 --out backtest/resultsp3 [--procs 4] [--reps 200] [--shuffles 200] [--shuffle-mode day|month]
...
三個對照（登錄 §三，缺一不可）：
  ...
  丙 同平均曝險、時點隨機：取甲每個種子的逐日曝險 e_t＝hold_val/equity 與持股部分逐日報酬 r_inv_t＝(equity_t/equity_{t−1}−1)/e_{t−1}
     （e_{t−1}＝0 的日子 r_inv＝0、另計筆數），把 e 隨機重排（day＝整段逐日；month＝以月為塊）套回同一條 r_inv：
     equity'_t＝equity'_{t−1}×(1＋e'_{t−1}×r_inv_t)。平均曝險與甲相同、選股報酬流相同，只有「哪幾天空手」被打散；每種子重抽 K 次（新種子 9000+k）。
格子（登錄 §四）：主格＝P1 贏過 0050 的三格 AND N30 null／N40 null／N40 relvol（d=∞、H60 出場）＋ N ≤ 8 的全部格（P1 網格）；其餘不跑。
...
def _init(sig, closes, opens, ncal, first_all, split_pos, end_all, bench, month_ids, shuffles, mode):
    _S.update(sig=sig, closes=closes, opens=opens, ncal=ncal, first_all=first_all, split_pos=split_pos, end_all=end_all, bench=bench,
              month_ids=month_ids, shuffles=shuffles, mode=mode)


def sim_cell_seed(args):
    """一格一種子：甲、乙各跑一次；丙 K 次重排。回傳逐種子列。"""
    N, d, rule, seed = args
    ...
    mids = _S["month_ids"][fa:ea]
    mdds, cagrs = [], []
    for k in range(_S["shuffles"]):
        rng = np.random.default_rng(SHUFFLE_SEED0 + k)
        eq2 = rebuild(shuffle_exposure(e, rng, _S["mode"], mids), r_inv)
    ...
    out = {"N": N, "d": d if d else np.inf, "rule": rule, "seed": seed,
           ...
           **{f"B_{k}": v for k, v in sb.items()}, "B_slot": B["slot_use"], "B_trades": B["trades"],
           "avg_exposure": float(e.mean()), "zero_exp_days": n_zero, "days": int(ea - fa),
           "C_mdd_p10": float(np.quantile(mdds, 0.1)), "C_mdd_p50": float(np.median(mdds)), "C_mdd_p90": float(np.quantile(mdds, 0.9)),
           "C_cagr_p10": float(np.quantile(cagrs, 0.1)), "C_cagr_p50": float(np.median(cagrs)), "C_cagr_p90": float(np.quantile(cagrs, 0.9)),
           "A_mdd_pct_in_C": float((mdds < sa["mdd"]).mean() * 100),     # 甲的回落比幾 % 的丙深（回落是負數：mdds < 甲 ⇒ 丙更深）
           "A_cagr_pct_in_C": float((cagrs < sa["cagr"]).mean() * 100)}
    return out
...
def write_csv(df: pd.DataFrame, path: str, stamp: str, commit: str, mode: str):
    with open(path, "w", encoding="utf-8") as fh:
        fh.write(f"# commit={commit} run={stamp} (Asia/Taipei) prereg=backtest/PREREGP3.md seeds={SEED0}+r shuffle_seeds={SHUFFLE_SEED0}+k shuffle_mode={mode}\n")
...
def main():
    ...
    ap.add_argument("--shuffles", type=int, default=200); ap.add_argument("--shuffle-mode", choices=["day", "month"], default="day")
    ...
    rows, cell_rows, summ = [], {}, []
    with Pool(a.procs, initializer=_init, initargs=(AND, closes, opens, ncal, first_all, split_pos, end_all, bench, month_ids, a.shuffles, a.shuffle_mode)) as pool:
        for cell in cl:
            N, d, rule = cell
            ...
            r = {"cell": "主格" if cell in MAIN_CELLS else "N≤8", "N": N, "d": d if d else np.inf, "rule": rule, "months": months, "reps": len(df), "med_seed": med_seed,
                 ...
                 **{f"{p}_{k}_p{q}": df[f"{p}_{k}"].quantile(q / 100) for p in ("A", "B") for k in ("cagr", "mdd") for q in (10, 90)},
                 ...
                 "avg_exposure_med": md["avg_exposure"], "zero_exp_days_med": md["zero_exp_days"],
                 "C_mdd_p10_med": md["C_mdd_p10"], "C_mdd_p50_med": md["C_mdd_p50"], "C_mdd_p90_med": md["C_mdd_p90"],
                 "C_cagr_p50_med": md["C_cagr_p50"], "A_mdd_pct_in_C_med": md["A_mdd_pct_in_C"], "A_cagr_pct_in_C_med": md["A_cagr_pct_in_C"],
                 "bench_cagr": b["c"], "bench_mdd": b["m"], **jd}
            summ.append(r)
            print(f"  N={N} d={d or '∞'} {rule}: 甲 {md['A_cagr'] * 100:+.1f}%／{md['A_mdd'] * 100:.1f}% 乙 {md['B_cagr'] * 100:+.1f}%／{md['B_mdd'] * 100:.1f}% 0050 {b['c'] * 100:+.1f}%／{b['m'] * 100:.1f}% 甲在丙回落百分位 {md['A_mdd_pct_in_C']:.0f}  {jd['judge']}  {time.time() - t0:.0f}s", file=sys.stderr, flush=True)
    S = pd.DataFrame(summ); write_csv(S, os.path.join(a.out, "summary.csv"), stamp, commit, a.shuffle_mode)
    write_csv(pd.concat(rows, ignore_index=True), os.path.join(a.out, "seeds.csv"), stamp, commit, a.shuffle_mode)
    write_csv(yearly_table(cell_rows, cal, bench), os.path.join(a.out, "yearly.csv"), stamp, commit, a.shuffle_mode)
    main_ = S[S["cell"] == "主格"]; small = S[S["cell"] == "N≤8"]
    L = [f"# PREREGP3 閒置資金放 0050 vs 空手——細表", "",
         f"產出：{stamp}（台北）、commit {commit}。判準 `backtest/PREREGP3.md`。種子 {SEED0}+r（沿用 P1，r<{a.reps}）；丙重排種子 {SHUFFLE_SEED0}+k（k<{a.shuffles}，mode={a.shuffle_mode}）；成本 {COST}、bench 單邊 {COST / 2}；期間 {cal[first_all].date()}～{cal[end_all - 1].date()}（{months} 個月）。0050 買進持有：年化 {b['c'] * 100:+.1f}%、最大回落 {b['m'] * 100:.1f}%。", ""]
    L.append("## 一、主格三格（P1 贏過 0050 的那三格）"); L.append("")
    L.append("| 格 | 甲 年化／回落（中位） | 乙 年化／回落（中位） | 乙−甲 回落 中位（p10～p90） | 槽位 甲／乙 | 平均曝險 | 丙 回落 p10／p50／p90 | 甲回落在丙的百分位 | P1 win 甲／乙 | 判定 |"); L.append("|---|---|---|---|---|---:|---|---:|---|---|")
    for r in main_.itertuples():
        L.append(f"| N{r.N} {r.rule} | {r.A_cagr_med * 100:+.1f}%／{r.A_mdd_med * 100:.1f}% | {r.B_cagr_med * 100:+.1f}%／{r.B_mdd_med * 100:.1f}% | {r.BminusA_mdd_med * 100:+.1f}pp（{r.BminusA_mdd_p10 * 100:+.1f}～{r.BminusA_mdd_p90 * 100:+.1f}） | {r.A_slot_med * 100:.0f}%／{r.B_slot_med * 100:.0f}% | {r.avg_exposure_med * 100:.0f}% | {r.C_mdd_p10_med * 100:.1f}／{r.C_mdd_p50_med * 100:.1f}／{r.C_mdd_p90_med * 100:.1f}% | {r.A_mdd_pct_in_C_med:.0f} | {r.win_A}／{r.win_B} | {r.judge} |")
    ...
    L.append(""); L.append("⚠ 甲在丙的百分位：回落是負數，「甲比 X% 的丙深」＝丙中比甲淺的占 100−X%；丙保持平均曝險、打散時點，⛔ 不保留曝險的時序結構（登錄 §五④）。")
