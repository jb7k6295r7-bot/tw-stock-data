...
def verdict_i(d_abs: float, ci: dict) -> tuple[bool, str]:
    ...
    # ⭐⭐ 第三種情形【已裁】⇒ 以下是定版措辭【逐字照抄】，⛔ 回測線一個字都沒有自己發明
    #   來源：K線分析線 2026-09-22 17:03 §二（裁定一）
    #   ＋ 策略線《P16 結論解讀與結案措辭》seq=2（sha c8ff0c649681d894）§3-3 收斂後採其逐字
    #   ⛔ 登錄 §三(i) 原本只列「含 0」與「明顯大於」兩種 ⇒ 本情形是【登錄沒有列舉的第三種】
    return False, ("⛔⛔ CI 不含 0，但【方向相反】⇒ ⭐ 登錄沒有列舉的【第三種情形】（✅ 已裁）\n"
                   "\n"
                   "【定版措辭・⛔ 引用時整段抄】（K線分析線 1703 §二／策略線 解讀 seq=2 §3-3）：\n"
                   ...
                   "  本身額外有害，而非中性』。」\n"
                   "\n"
                   "⛔⛔【引用那一段時必須同時附上】策略線的四條披露（⭐ 往更嚴的方向補）：\n"
                   "  ② 配對差 −2.40pp、CI[−3.16, −1.63]、n＝200、【為正 60 顆】⇒ ⛔ 四個數字都要報\n"
                   "  ③ 範圍 ＝ 755 筆／200 顆種子／一個窗 ⇒ ⛔「額外有害」只在這個範圍上成立\n"
                   "     ⇒ ⛔⛔【顯著】指的是【逐種子配對差】那個量，⛔ 不是回落本身\n"
                   "       （回落是一窗一個數 ⇒ ⛔ 不可用 200 顆種子講回落本身的信心水準）\n"
                   "  ④ ⛔ 不可據它宣告反向做法（訊號消失就加碼／續抱）\n"
                   "  ⑤ ⛔ 措辭裡不可出現「改善」二字（兩側都是負的 ⇒ ⭐ 那是算術，不是修辭）\n"
                   "\n"
                   "⭐ (i) 的獨立性（〈一百二十五〉）在本件【沒有退化】：R1 自己的效果量不是 0\n"
                   "   （回落 −29.28pp、曝險 0.305 vs E0 0.871）\n"
                   "⚠ 而本件與 P12／P14／P15 共用選股種子（102000+r）⇒ ⛔ 不是互相獨立的證據（〈七十八〉）")
...
def report(res: dict) -> dict:
    ...
    A("")
    A(f"⭐⭐【裁定二（甲）・K線分析線 1703 §三／策略線 解讀 seq=2 §4-2】本欄逐字標：")
    A(f"   **【結構上不可得（共同區間恆為 {max(x['共同區間日數'] for x in pb_rows)} 個交易日）・驗到 "
      f"{pb['共同區間可驗']} 顆】**")
    A("   ⛔ 不可讀成「已驗過」、⛔ 不可讀成「通過」、⛔ 也不可讀成「不通過」")
    A("   ⭐ 並且引用時必須連【它原本要擋什麼】一起寫（⛔ 不可只寫「那一欄不可得」）")
    A("")
    A("⇒ ⛔⛔ 所以本件的結論只能寫：**【三臂在共同區間內完全相同】這句話，本件沒有驗到**")
    A("   ⛔ 不是驗過而不同，是那一欄【驗了 0 顆】")
    A("   ⛔ 更不可寫【三臂在第一次出場前完全相同】（§十二② 逐字）")
    A("")
    ...
    A("    ⇒ ⭐ 第一次出場幾乎貼著窗首 ⇒ 共同區間【結構上】短到驗不了東西")
    A(f"  ⇒ ⛔⛔ 所以這一欄實際只驗到 {pb['共同區間可驗']}/{pb['n']} 顆種子")
    A("    ⛔ 它擋不住「第二次進場就分岔」那一類錯誤 —— ⚠ 而那正是 §十一① 說要它來擋的東西")
    A(f"  ⇒ ⭐ 本件真正擋住那一類錯誤的是【§四⑦ 那一欄】：全 {reps} 顆種子、")
    A("    第一個進場日的 (持股清單, 買價) 逐位元相同（見上一小節）")
    A("")
    A("⭐⭐【裁定二・本件的把關到哪裡為止】（策略線 解讀 seq=2 §4-4，⛔ 逐字）：")
    A("  ✅ 擋住了：第一個進場日的 (持股清單, 買價) sha256 ⇒ 五臂逐位元相同、200 顆")
    A("  ✅ 擋住了：各臂對 E0・各自區間 ⇒ 200/200 逐位元相同")
    A("     ⛔ 而它【只能講該臂自己】，⛔ 不可用它說「三臂相同」（§十二② 逐字）")
    A("  ⛔ 沒有宣告擋住的：「第二次進場就分岔」那一類")
    A("     ⇒ ⭐ 因為交件【沒有報】各臂自己那個區間裡有幾筆進場")
    A("     ⇒ ⛔ 依〈八十〉：那是【沒有被報】，⛔ 不是【被報成 0】⇒ absent 的語意只能證偽")
    A("  ⇒ ⭐ 所以最強只能寫【沒有【已知的】缺口】，⛔ 不可寫【沒有缺口】")
    A("     （策略線 seq=2 §4-4 的收窄，⏳ 待 K線分析線 過目確認）")
    A("  ⇒ ⛔⛔ 而本件【不補跑那一欄】（跑完了才加一欄 ＝ 事後加格）")
    A("     ⇒ ✅ 改為往前適用：下一份登錄的必填欄（§4-3①）")
    A("  ⇒ ⚠ 而要一起寫的一句【對本線不利也照寫】：那兩欄是【意外具備】的")
    A("     ⇒ ⭐ 本件不是「設計得好」，是【運氣好】")
    A("```")
    A("")
    A("---")
    ...
    if imp_e1.median() < 0 and imp_r1.median() < 0:
        A("⚠⚠ 兩個「改善」都是【負的】⇒ ⛔ 這一道比的是【兩個惡化誰比較輕】，⛔ 不是誰改善得多")
        A("   ⇒ ⭐ 而 E1 惡化得【比 R1 更多】（⛔ 這一句是算術，⛔ 不是解讀）")
