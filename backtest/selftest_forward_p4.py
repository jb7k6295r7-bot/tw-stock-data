...
if __name__ == "__main__":
    ...
    try:
        r = run(["--date", "2026-08-01", "--out", out, "--limit", "60", "--procs", "2"])
        ...
        check(rec["type"].isna().all() and (rec["centers_version"].isna() | (rec["centers_version"] == "")).all(), "沒中心 ⇒ 型號空")
        ...
        n1 = len(rec)
        r2 = run(["--date", "2026-08-15", "--out", out, "--limit", "60", "--procs", "2"])
        ...
        r3 = run(["--date", "2026-09-01", "--out", out, "--limit", "60", "--procs", "2"])
...
# 突變：把 records 的冪等檢查拿掉 ⇒ 第 7 條紅；update_universe 改成整檔取代 ⇒ 第 9 條紅
