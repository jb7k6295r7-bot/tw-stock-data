...
def report(tab, xtab, eff, att, anc, dirc, lad, ev, sigs, closes, opens, cal, wins, marks, reps, e0, n_670, own_med, secs) -> list:
    ...
    L += ["", f"⇒ ⭐⭐ **S 兩版**（主格窗、含成本）：全部 {float(s_row.loc['全部', 'point_pp']):+.2f}pp"
          ...
          "**① S 的『全部』版含著同樣那兩格 4 檔的格子** —— S 的主效果是",
          "　 在 (C,T) 四種組合上平均，其中兩個組合是 T0 ⇒ 它同樣吃進 (S1,·,T0) 那兩格。",
          "　 ⭐ 本線 1645／1740 把這件事報上去 ⇒ **策略線 1830 §三② 裁定、K線分析線 1845 §二 確認**：",
          "　 【S 比照 T 報兩版，⛔ 不含那兩格才進判定】⇒ ⭐ 上表的【只 T1】那一列就是判定版。",
          "　 ⛔ 而它**不是**回測線看過結果之後自己加的切法（〈六十四〉）——是兩線的裁定，本線照做。",
          ...
          "　⇒ ⭐ 那一格【4 檔】贏了 44pp。",
          f"　 ⇒ ⭐⭐ 而拿掉那一欄之後，S 的點估計從 {float(s_row.loc['全部', 'point_pp']):+.2f}pp **翻成** "
          f"{float(s_row.loc['T1', 'point_pp']):+.2f}pp ⇒ ⛔ 「門檻B 在這一段贏了全市場」那句話"
          "**只在含那兩格時才成立**；單看 T1 那一側它是輸的。",
