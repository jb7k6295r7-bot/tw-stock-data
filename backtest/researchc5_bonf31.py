# -*- coding: utf-8 -*-
"""批2 結算備用（⛔ 結算由裁定線做；本支只算數）：C5 出口② 的兩格（BTC、ETH）在累計 N＝31（裁定線 seq124 §二）下的 Bonferroni 調整 CI。
...
for s in ("BTC", "ETH"):
    ...
    np.save(f"backtest/resultsc5/bonf31_samples_{s}.npy", smp)
    print("{}：B＝{:,}、L＝{}｜Bonferroni（N＝31）CI [{:+.2%}, {:+.2%}] ⇒ 下界 {} 0".format(s, B, L, lo, hi, ">" if lo > 0 else "≤"))
json.dump(out, open("backtest/resultsc5/bonferroni_N31.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1, default=float)
