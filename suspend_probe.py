...
def probe_longhalt(say, sleep):
    ...
    ## ⭐ 「保留窗口有多長」這個問題，**答案在 main 的 git 歷史裡**

    2026-09-16 量到：`mopsfin_t187ap26_O` 回的**不是**「今天發生的事件」，
    是「**現在還有效的案件**」——那一列是 4804 大略-KY，
    `停止買賣開始日` **1150903**（＝ 13 天前），`Date` 卻是**今天**。

    ⇒ ⭐ 所以每天抓一次撿得到的不只是「今天新增的」，
    ⛔ 而「它保留多久」我**只有兩天的觀測**（09-15、09-16，兩天同一列）⇒ **未驗**。

    ⚠ 而要量它**不必新增台帳**：這個輸出檔每一趟都推上 main
    ⇒ `git log -p origin/main -- data/meta/_suspend_probe.txt` 逐版看
    `(代號, 停止買賣開始日)` 什麼時候消失，**那就是保留窗口**。
    ⇒ ⛔ 不要另開一份累積檔——續跑判準一律用**資料自己**（第四點）。
    """
