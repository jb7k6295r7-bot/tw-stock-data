...
KNOWN = {
    ...
    # 2026-09-13 `_reduce_ratio_probe.txt`：TWSE exRight/TWT49U（除權除息計算結果表）
    # ⭐ 而這兩條式子我方 2026-09-12~13 推了兩輪（`reduce_shares_check` 的
    #   「兩種除權算式」）——⚠ 官方每天都在同一個回應裡給。
    # ⛔ 而我推的那版少了 **息值**：我把「除權」與「除息」當成兩件事分開推，
    #   官方這一條是**同一條式子同時處理**（權息合併的案子才是常態）。
    "exright": [
