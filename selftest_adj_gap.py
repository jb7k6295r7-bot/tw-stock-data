...
def main():
    ...
    # ⛔⛔ 2026-09-14：這一條本來要求 `未歸因` **一定要在**類別裡。
    #   ⚠ 而 `_adj_gap_low.txt` 已經是 **0** ⇒ 未歸因收斂到 0 是這一族的**目標**，
    #   ⇒ 目標達成的那一天，這條斷言**必然**失敗 ⇒ 而它在 daily.yml 裡是
    #     `continue-on-error: true` ⇒ ⛔ **沒有任何地方會說**。
    #   （實測：main 上 3 筆 ⇒ 綠；分支上 0 筆 ⇒ 紅。同一支程式、同一天。）
    #   ⇒ ⭐ 判準改成「**有歸因欄，而且至少分得出一種類別**」，
    #     ⛔ 不是「一定要有未歸因這個樣本」。
    ck("有歸因欄且分得出類別",
       {r["why"] for r in rows} >= {"事件掛在休市日"},
       str({r["why"] for r in rows}))
    ck("第一趟斷言不判定", "**✗**　未歸因的筆數 **沒有變多**" not in t1)
    ...
    if n_un < 2:
        # ⛔⛔ 這個情境是拿**現場資料**造的：要「本輪 > 偽造的水位 1」，
        #   就得現場至少有 2 筆未歸因。⚠ 而未歸因收斂到 0 是**目標**
        #   ⇒ 這一層在達成目標的 ref 上**必然**造不出來。
        #   CLAUDE.md 六點五：一條在某個環境下必然不成立的斷言，
        #   等於把那個環境的整條線關掉 ⇒ ⭐ 大聲印，⛔ 不算失敗。
        print(f"  ⚠⚠ **這一層沒跑**：現場只有 {n_un} 筆未歸因，"
              "造不出「本輪高於水位」的情境　⇒ ⛔ 不算失敗，⛔ **也不算驗過**")
        # ⇒ 而「跳掉之後還有沒有人在守？」**有**，而且比這裡強：
        #   `selftest_lowwater.py` ②⑦ 拿**合成**數字驗同一條判準
        #   （水位 10／本輪 12 ⇒ down 必紅），⭐ 不依賴現場有幾筆未歸因
        #   ⇒ 每一個 ref 上都會跑。
        print("      ⇒ 守這一條的是 `selftest_lowwater.py` ②⑦（合成數字，不依賴現場）")
    else:
        t2 = run(out2, lr2, low2, cached=True)
        ck("★ 未歸因高於歷史最低值 ⇒ 斷言變 ✗",
           "**✗**　未歸因的筆數 **沒有變多**" in t2,
           [l for l in t2.splitlines() if "未歸因的筆數" in l])
        ck("★ ✗ 的細節寫得出前後值", f"本趟 {n_un:,} vs 歷史最低 1" in t2,
