...
## 一點五、⚠ 2026-09-19 使用者接了幾個 MCP 連接器，⛔ 逐一核過，沒有一個解決現有的卡點

`ListConnectors` 現況：Firecrawl／Google Drive／PDF Viewer／Spotify（四個都是 connected＋enabledInChat）。

```
Firecrawl     ⭐ 值得一試：它是**另一條網路路徑**（不經過本容器的閘道），
              而且帶抓取／解析，⚠ 而它**是不是能執行 js**沒有驗過
              ⇒ 下一個 session 若要碰 C4（面額變更自動取得），
                先拿它對一個**已知答案**的頁面試（例如面額變更公告頁）、
                比對抓到的內容跟 `data/adj/` 裡已經人工核過的 14 筆，
                ⛔ 不要直接拿它的結果當供料——先驗才能信（CLAUDE.md 第一點的同一條路）。
Google Drive  ⛔ 本線一直都在用（收件夾），不是新的
PDF Viewer／Spotify  ⛔ 跟本專案的資料型態無關，不預期用得到
```

⇒ ⭐ 判準沒有變：**换供料是換維護者的決定**（第五點）——即使 Firecrawl 真的能拿到
C4 或 D2 的資料，接不接、算不算供料一樣要先量、再送市場情報分析線裁，
⛔ 不是「工具能用就直接接上去」。
