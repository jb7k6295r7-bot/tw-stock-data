...
#
#     3141 民106/108/109/110/112/113 **六年全部**是捨去
#     2305 民104/105/107/108/109     **五年全部**是四捨五入
#
# ## ⇒ ⭐⭐ 而改用**全母體**（17,832 個 (檔,年)，⛔ 不是抽樣）之後，
#      我從那兩檔推的「價位／檔位」假說**當場被推翻**
#
#     官方 − 四捨五入(我方均值) 的分佈：
#        0.00   13,460  **75.48%**
#       −0.01    4,193  **23.51%**   分的小數部分 ∈ [0.5000, 0.9960]（≈均勻）
#       +0.01      179   **1.00%**   分的小數部分 ∈ [0.4504, 0.4980]
#     ⇒ ⛔ **沒有任何一格超過 ±0.01**
#
#     逐檔純度：只捨去(≥3年) **715** 檔｜只四捨五入(≥3年) **700** 檔｜混合 179 檔
#              ⇒ **88.8%** 純 ⇒ ⭐ 它確實是**逐檔**的
#
# ⛔ 而兩群的**價位分佈幾乎一樣**：年均價中位數 31.16 vs 29.46，
#   範圍 [1.72, 4578] vs [1.10, 5028] ⇒ ⭐ **價位假說死了**。
#
# ⛔ 第二個也死了：「官方的分母是**全市場交易日數**而不是該股有成交天數」
#   ——只看有缺日的那 3,500 格，這個算法命中 **0.00%**（該股天數 65.49%）。
#
# ## ⇒ ⭐⭐⭐ 而 +0.01 那 179 格是**唯一**推得出硬結論的一批
#
# 它們的「分的小數部分」最大 0.4980 ⇒ ⛔ **任何我方均值的進位規則都做不出 +0.01**
# ⇒ ⭐ 官方的 Σ 在那 179 格**必定**跟我方不同。
# ⚠ 而 −0.01 那 4,193 格的小數幾乎均勻鋪在 [0.5, 1) ⇒ ⛔ 它們**單獨**看
#   跟「官方就是無條件捨去」完全相容 ⇒ 兩批**不一定是同一個成因**。
#
# ## ⇒ ⭐⭐ 而 179 那一批有一個**時點**
#
#     民104~106（2015~2017）  **0 個**
#     民107 起（2018~）        每年 16~28 個
#   ⇒ ⭐ 那不是慢慢變多，是**從某一年開始才有** ⇒ 有一件事在那時換了。
#   ⛔ 換了什麼我不知道。
#
# ⚠ 順帶記一個**查過的巧合**：「+0.01 有 179 格」與「混合檔有 179 檔」
#   數字一樣 ⇒ ⛔ 我去查過**不是同一批**（交集只有 23 檔）——是巧合。
#
