...
# ═════════════════════════════════════════════════════════════════
# ⛔⛔ 「年均價 ±0.01」那一節（`avg_residual()` 三個錨點逐日比對）
#    **2026-09-16 拿掉了**——⭐ 因為答案出來了，而且不需要連外。
#
#    答案：**進位規則，三段**（上櫃 trunc 100%／上市民107~ banker's 100%／
#    上市民104~106 直接四捨五入 99.88%）⇒ 全母體 17,829／17,832 逐位相同。
#    ⭐ 實作在 `official_stats.avg_close_expected()`（唯一一份），
#      形狀與限制在 `docs/READ_CONTRACT.md` 那一節。
#
# ⛔⛔⛔ 而我原本寫在這裡的那段推論是**錯的**，錢在哪裡要留下來：
#
#    我寫：「雙重進位**修好**的 50 個與**弄壞**的 57 個，
#    小數部分落在同一個帶 [0.45,0.50) 而且幾乎同分佈
#    ⇒ ⛔ **任何「平均值的函數」都做不到這件事**⇒ 進位那一族被排除。」
#
#    ⛔ 而它真的是一個平均值的函數（banker's）。實測：
#
#      民107~、d ∈ [0.45,0.50) 的 345 格：官方進位 179｜沒進位 166
#        在 **d** 這個軸上：min 0.4504 p50 0.4793 max 0.4980
#                        vs min 0.4504 p50 0.4793 max 0.4979  ⇒ ⛔ **分不開**
#        ⭐⭐ 而在**進位後「分」位的奇偶**這個軸上：
#                        偶數 **179／179** vs **166／166** ⇒ ⭐ **當場分開**
#
# ⇒ ⭐⭐⭐ 通則（已收進 CLAUDE.md）：
#    **「兩批在我選的那個軸上同分佈」只證明「這個軸分不開它們」，
#    ⛔ 不證明「沒有軸分得開」——而我拿前者寫了後者，
#    ⭐ 還據此**排除了正確答案所在的整一族**。
#
# ⚠ 而拿掉而不是「標註別用」的理由是四點五那條：
#    它每一趣要打 36 發 TWSE（3 個錨點 × 12 個月）來回答一個**已經有答案**的問題，
#    ⛔ 而下一個人看到它的輸出會以為這件事還沒有答案。
# ═════════════════════════════════════════════════════════════════
...
# ⚠ `_same_price()` 與 `_our_closes()` 跟著 `avg_residual()` 一起拿掉了（上面那一段）。
#   ⛔ 而它們的教訓要留著，因為下一個要「拿官方價格跟我方比」的人會再寫一次：
#
#     官方回 `4.30`，我方 CSV 寫 `4.3` ⇒ ⛔⛔ **比字串會證實一個不存在的發現**
#       （每一天都會被判成「收盤不同」，而「有幾天差 0.01」正是要找的答案）
#     ⇒ ⭐ 比**數值**，而且回**三種**答案：相同／不同／⭐ **比不了**（五點三那條的形狀）。
