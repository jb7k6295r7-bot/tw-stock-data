...
TARGETS = [
    ...
    ("上市三大法人 T86",
     f"{TW}/fund/T86?date={YMD}&selectType=ALLBUT0999&response=json"),
    ("上市融資融券 MI_MARGN",
     f"{TW}/marginTrading/MI_MARGN?date={YMD}&selectType=ALL&response=json"),
    ("上市外資持股 MI_QFIIS（上市 shares 的來源）",
     f"{TW}/fund/MI_QFIIS?date={YMD}&selectType=ALLBUT0999&response=json"),
    ("上市除權息 TWT49U",
     f"{TW}/afterTrading/TWT49U?date={YMD}&response=json"),
    ("上市暫停交易 TWTAWU",
...
def show(label, url):
    ...
    say(f"   頂層鍵 {sorted(d)}")
    say(f"   ⭐ 我方**丟掉**的鍵：{dropped}")
    for k in dropped:
        v = d[k]
        if isinstance(v, (list, tuple)):
            say(f"     ── {k}（list，{len(v)} 項）")
            for x in v[:8]:
                say(f"        {str(x)[:200]}")
        elif isinstance(v, dict):
            say(f"     ── {k}（dict）：{json.dumps(v, ensure_ascii=False)[:300]}")
        else:
            say(f"     ── {k}：{str(v)[:300]}")
    # ⭐ 三個數字對一次：官方說的 total、data 的長度、各表加總
    n_data = sum(len(t.get("data") or []) for t in tabs)
    tot = d.get("total")
    if tot is not None:
        ok = "✓" if str(tot).strip() == str(n_data) else "⚠ **對不上**"
        say(f"   ⭐ total={tot}｜我方解析出的列數={n_data}　{ok}")
    say("")
